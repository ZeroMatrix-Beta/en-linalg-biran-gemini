...
* **Inline Column Vectors:** To maintain consistent vertical spacing in prose, write column vectors as transposed row vectors, e.g., $(x_1, \dots, x_n)^T$, instead of using vertical stacks like `smallmatrix`. **Exception:** For $2 \times 1$ vectors in inline math, the use of `\binom{a}{b}` is permitted to match the professor's notation style for short vectors.

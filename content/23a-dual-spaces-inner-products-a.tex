...
To prove statement \textbf{(c)}, simply apply the orthogonal complement $(-)^\perp$ to statement \textbf{(b)}.
\end{proof}
...
\[
    [T^*]_{\mathcal{B}}^{\mathcal{C}} = \left( \overline{[T]_{\mathcal{C}}^{\mathcal{B}}} \right)^\top = \left([T]_{\mathcal{C}}^{\mathcal{B}}\right)^* \quad \text{(\newterm{adjoint matrix})}
\]
\end{proposition}
...
\begin{enumerate}[label=\textbf{(\alph*)}]
    ...
    \item $\overline{I_n}^\top = I_n^\top = I_n$ (since $I_n$ is a real matrix).

...
\begin{exercise}[Countable Bases and Independent Subsets]
\label{exc:countable_basis_independent_subsets}
Let $V$ be a vector space over a field $K$ that admits a countable basis $\mathcal{B} = \{v_1, v_2, \dots\}$. Show that every linearly independent subset $S \subseteq V$ is at most countable (i.e., finite or countable).
\exinfo{This exercise is Problem 5 of Problem Sheet 7, Linear Algebra I, Autumn Semester 2025.}
\exsol{sol:countable_basis_independent_subsets}
\end{exercise}
...
\exsol{sol:basis_of_function_space}
\end{exercise}
...
\label{sol:basis_of_function_space}
...
\input{content/11-bases-part-b-solutions.tex}

\begin{ainote}
Two lemmas prepare the Basis Theorem, and the first of them,
\cref{lem:no_predecessor_span}, closes a gap that is easy to walk past. The
filter that extracts a basis from a spanning list only ever tests a vector
against the ones \emph{preceding} it, whereas
\cref{prop:dependence_is_redundancy} speaks of a vector lying in the span of
\emph{all} the others. Those are different conditions, and it is the first that
the extraction argument needs; \cref{lem:spanning_list_contains_basis} is where
the two are finally brought together.

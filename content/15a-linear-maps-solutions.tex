...
\exinfo{Official Solution (Problem Sheet 9, Exercise 1). Proves non-linearity of extending a non-trivial map by zero outside a proper subspace.}
...
\exinfo{Official Solution (Problem Sheet 9, Exercise 2). Establishes $\mathbb{Q}$-homogeneity for additive maps using induction and scaling.}
...
\exinfo{Official Solution (Problem Sheet 9, Exercise 3). Identifies parameter values making integral and point-evaluation maps linear via test polynomials.}
...
\exinfo{Solution by Gemini 3.7 Flash (High). Based on Problem Sheet 9, Multiple Choice. Analyzes linearity of vector formulas and matrix operations.}
\begin{enumerate}[label=\textbf{(\arabic*)}]
...
\exinfo{Solution by Gemini 3.7 Flash (High). Based on Problem Sheet 9, Multiple Choice. Identifies the canonical linear transformation associated with a $3 \times 2$ matrix.}
...
\exinfo{Official Solution (Problem Sheet 9, Exercise 4). Proves preimage of a subspace is a subspace and derives its dimension formula via Rank-Nullity on the restricted map.}
...
\exinfo{Official Solution (Problem Sheet 9, Exercise 5). Proves isomorphisms preserve linear independence, spanning sets, and bases.}
...
\exinfo{Official Solution (Problem Sheet 9, Exercise 6). Explains the canonical scalar isomorphism $\operatorname{End}(V) \cong K$ in dimension 1 versus non-canonical $V \cong K$.}
\begin{enumerate}[label=\textbf{(\alph*)}]
...
\exinfo{Official Solution (Problem Sheet 9, Exercise 7). Demonstrates that linear functionals with identical nullspaces are scalar multiples of each other.}
...
\exinfo{Official Solution (Problem Sheet 9, Exercise 8). Establishes the 1-to-1 correspondence between direct sum decompositions and projection operators.}
\begin{enumerate}[label=\textbf{(\alph*)}]
...
\exinfo{Hybrid Solution (Gemini 3.7 Flash / Official Problem Sheet 9, Exercise 9). Deduces invertibility of the middle endomorphism $T$ when $S \circ T \circ U = \id_V$ in finite dimensions.}
...
\exinfo{Official Solution (Problem Sheet 9, Exercise 10). Evaluates statements on automorphism extension, image-kernel complements, and parity of rank-nullity dimensions.}
\begin{enumerate}[label=\textbf{(\alph*)}]

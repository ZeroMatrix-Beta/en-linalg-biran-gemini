...
\exinfo{Official Solution (Problem Sheet 10, Exercise 2). Proves non-linearity of extending a non-trivial map by zero outside a proper subspace.}
\end{exercisesolution}

% Official Solution (Problem Sheet 10, Exercise 7)
...
\exinfo{Official Solution (Problem Sheet 10, Exercise 7). Establishes $\mathbb{Q}$-homogeneity for additive maps using induction and scaling.}
\end{exercisesolution}

% Official Solution (Problem Sheet 9, Exercise 2)
...
\[
T_1(p) = 3p(4) + 5p'(6) + b p(1)p(2), \qquad T_2(p) = \int_{-1}^2 x^3 p(x) \, dx + c (p(0))^2.
\]
\begin{itemize}
    \item \textbf{If $b = c = 0$:} The components become:
    \[
    T_1(p) = 3p(4) + 5p'(6), \qquad T_2(p) = \int_{-1}^2 x^3 p(x) \, dx.
    \]
    Evaluation at a point ($p \mapsto p(x_0)$), differentiation followed by evaluation ($p \mapsto p'(x_0)$), and definite integration against a fixed polynomial weight function are well-known linear functionals on $\mathbb{R}[x]$. Any linear combination of linear functionals is linear, so $T_1$ and $T_2$ are linear, making $T$ linear.
    ...
    Consider the constant polynomial $p_0(x) = 1$ (whose derivative is $p_0'(x) = 0$):
    \[
    T_1(p_0) = 3(1) + 5(0) + b(1)(1) = 3 + b, \qquad T_1(2 p_0) = 3(2) + 5(0) + b(2)(2) = 6 + 4b.
    \]
    By linearity, $T_1(2 p_0) = 2 T_1(p_0) = 6 + 2b$. Equating the two yields $6 + 4b = 6 + 2b \implies 2b = 0 \implies b = 0$.

    Similarly for $T_2$, using $\int_{-1}^2 x^3 \, dx = \left[\frac{x^4}{4}\right]_{-1}^2 = \frac{16 - 1}{4} = \frac{15}{4}$:
    \[
    T_2(p_0) = \frac{15}{4} + c(1)^2 = \frac{15}{4} + c, \qquad T_2(2 p_0) = 2\left(\frac{15}{4}\right) + c(2)^2 = \frac{15}{2} + 4c.
    \]
    By linearity, $T_2(2 p_0) = 2 T_2(p_0) = \frac{15}{2} + 2c$. Equating the two yields $\frac{15}{2} + 4c = \frac{15}{2} + 2c \implies 2c = 0 \implies c = 0$.
\end{itemize}
Thus, $T$ is linear if and only if $b = c = 0$.
\exinfo{Official Solution (Problem Sheet 9, Exercise 2). Identifies parameter values making integral and point-evaluation maps linear via test polynomials.}
\end{exercisesolution}

% Official Solution (Problem Sheet 9, Multiple Choice Question 1)
...
\end{enumerate}
\exinfo{Official Solution (Problem Sheet 9, Multiple Choice Question 1). Analyzes linearity of vector formulas and matrix operations.}
\end{exercisesolution}

% Official Solution (Problem Sheet 9, Multiple Choice Question 2)
...
\]
Therefore, map \textbf{(a)}, $f \colon \mathbb{R}^2 \to \mathbb{R}^3, f(x, y) = (2x + y, x, 2y)\transp$, is the correct one.
\exinfo{Official Solution (Problem Sheet 9, Multiple Choice Question 2). Identifies the canonical linear transformation associated with a $3 \times 2$ matrix.}
\end{exercisesolution}
...
\begin{enumerate}[label=\textbf{(\alph*)}]
    ...
    \]
\end{enumerate}
\exinfo{Official Solution (Problem Sheet 10, Exercise 5). Proves preimage of a subspace is a subspace and derives its dimension formula via Rank-Nullity on the restricted map.}
\end{exercisesolution}

% Official Solution (Problem Sheet 10, Exercise 6)
...
\end{enumerate}
\exinfo{Official Solution (Problem Sheet 10, Exercise 6). Proves isomorphisms preserve linear independence, spanning sets, and bases.}
\end{exercisesolution}

% Official Solution (Problem Sheet 10, Exercise 1)
...
\begin{enumerate}[label=\textbf{(\alph*)}]
    ...
    \end{itemize}
\end{enumerate}
\exinfo{Official Solution (Problem Sheet 10, Exercise 1). Explains the canonical scalar isomorphism $\operatorname{End}(V) \cong K$ in dimension 1 versus non-canonical $V \cong K$.}
\end{exercisesolution}
...
\end{itemize}
\exinfo{Official Solution (Problem Sheet 11, Exercise 6). Demonstrates that linear functionals with identical nullspaces are scalar multiples of each other.}
\end{exercisesolution}
...
\begin{enumerate}[label=\textbf{(\alph*)}]
    ...
    \end{itemize}
\end{enumerate}
\exinfo{Official Solution (Problem Sheet 10, Exercise 4). Establishes the 1-to-1 correspondence between direct sum decompositions and projection operators.}
\end{exercisesolution}

% Solution: Gemini / Official Hybrid (Problem Sheet 12, Exercise 2)
...
\]
\exinfo{Hybrid Solution (Gemini 3.7 Flash / Official Problem Sheet 12, Exercise 2). Deduces invertibility of the middle endomorphism $T$ when $S \circ T \circ U = \id_V$ in finite dimensions.}
\end{exercisesolution}
...
\begin{enumerate}[label=\textbf{(\alph*)}]
    \item \textbf{Statement (a) is TRUE:}
    ...
    \item \textbf{Statement (b) is FALSE:}
    ...
    \item \textbf{Statement (c) is TRUE:}
...
\end{enumerate}
\exinfo{Official Solution (Problem Sheet 12, Exercise 4). Evaluates statements on automorphism extension, image-kernel complements, and parity of rank-nullity dimensions.}
\end{exercisesolution}

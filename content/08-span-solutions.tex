...
\exinfo{Official Solution (Problem Sheet 5, Multiple Choice). Verifies subspace span properties and intersection inclusions.}
...
\exinfo{Official Solution (Problem Sheet 5, Exercise 2). Proves two non-collinear vectors span $\mathbb{R}^2$ and classifies all subspaces of $\mathbb{R}^2$.}
...
\exinfo{Solution by Gemini 3.7 Flash (High). Solves the linear system for polynomial coordinates and proves the polynomials span $\mathbb{R}[x]_3$.}
\begin{enumerate}[label=\textbf{(\alph*)}]
...
\exinfo{Official Solution (Problem Sheet 5, Exercise 3). Proves $Ax = b$ subspace criterion requires $b=0$.}
...
\exinfo{Official Solution (Problem Sheet 5, Exercise 4). Demonstrates $K[x]$ is infinite-dimensional via the degree bound.}
...
\exinfo{Official Solution (Problem Sheet 5, Exercise 5). Constructs minimal generating sets for finite-support and eventually-constant sequence spaces.}

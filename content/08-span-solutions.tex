% Official Solution (Problem Sheet 7, Multiple Choice Question 1)
...
\begin{enumerate}[label=\textbf{(\arabic*)}]
    ...
    \item \textbf{True.} If $W \subseteq V$ is a linear subspace, then by \textbf{(LSS1)}--\textbf{(LSS3)} it contains all linear combinations of its elements, so $\Sp(W) \subseteq W$. Since $W \subseteq \Sp(W)$ always holds, we have $\Sp(W) = W$. Conversely, $\Sp(W)$ is always a linear subspace of $V$, so if $W = \Sp(W)$, then $W$ must be a linear subspace.
...
\end{enumerate}
\exinfo{Official Solution (Problem Sheet 7, Multiple Choice Question 1). Verifies subspace span properties and intersection inclusions.}
...
\begin{enumerate}[label=\textbf{(\alph*)}]
    ...
    \begin{itemize}
        ...
        \item If $U \ne \{(0, 0)\}$, there exists some non-zero vector $v \in U$. By \textbf{(LSS3)}, $\Sp(v) \subseteq U$.
        \begin{itemize}
...
\begin{itemize}
    ...
    If $U$ is a linear subspace, it must contain the zero matrix $0 \in \M_{m \times n}(K)$ by \textbf{(LSS1)}. Since $0 \cdot x = 0 \in K^m$ for any vector $x$, the condition $0 \in U$ forces $b = 0 \cdot x = 0$.
...
\exinfo{Official Solution (Problem Sheet 6, Exercise 4). Proves $Ax = b$ subspace criterion requires $b=0$.}
\end{exercisesolution}
...
\exinfo{Official Solution (Problem Sheet 6, Exercise 5). Demonstrates $K[x]$ is infinite-dimensional via the degree bound.}
\end{exercisesolution}

% Official Solution (Problem Sheet 6, Exercise 6)
...
\begin{enumerate}[label=\textbf{(\alph*)}]
    ...
    \end{itemize}
    Thus, $E = \{e_i \mid i \ge 0\}$ is a minimal generating set of $K_0^\infty$: no proper subset of it still generates. (Minimality is not the same as uniqueness. The family $\{e_0,\, e_0 + e_1,\, e_1 + e_2,\, \dots\}$ is another minimal generating set, and it is not obtained from $E$ by rescaling; what the argument above establishes is that $E$ cannot be shrunk, not that it is the only such set).
    ...
    \begin{itemize}
        ...
        \end{itemize}
    \end{itemize}
    Thus, $S = \{e_i \mid i \ge 0\} \cup \{\mathbf{1}\}$ is a minimal generating set of $K_{\mathrm{cst}}^\infty$, again in the sense that no proper subset of it generates.
\end{enumerate}
\exinfo{Official Solution (Problem Sheet 6, Exercise 6). Constructs minimal generating sets for finite-support and eventually-constant sequence spaces.}
\end{exercisesolution}

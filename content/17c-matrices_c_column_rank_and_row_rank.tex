...
Let $A \in \M_{n \times n}(K)$. Then, $A \in \GL_n(K)$ if and only if $A\transp \in \GL_n(K)$.
\end{lemma}
...
In summary, we have established that $A \in \GL_n(K)$ if and only if $\rank_{\text{row}}(A) = n$. But by definition, the row rank of $A$ is exactly the column rank of its transpose, so $\rank_{\text{row}}(A) = \rank_{\text{col}}(A\transp)$. Therefore, $A \in \GL_n(K)$ if and only if $\rank_{\text{col}}(A\transp) = n$, which means $A\transp$ is invertible.
\end{proof}

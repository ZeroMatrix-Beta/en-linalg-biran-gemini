...
\begin{enumerate}[label=\textbf{(\alph*)}]
    ...
    \item Taking the transpose of the equation $B = PAQ$ yields $B\transp = Q\transp A\transp P\transp$. Since $P$ and $Q$ are invertible matrices, their transposes $P\transp$ and $Q\transp$ are also invertible (we have previously seen that $(P\transp)^{-1} = (P^{-1})\transp$). We can now apply statement \textbf{(a)} of this \Cref{lemma:rank_invariance_equivalence}, which we have already proven, to the matrices $A\transp$ and $B\transp$. This gives us $\rank_{\text{col}}(B\transp) = \rank_{\text{col}}(A\transp)$. By the definition of the transpose, this immediately translates to $\rank_{\text{row}}(B) = \rank_{\text{row}}(A)$, since the columns of $A\transp$ are the rows of $A$, so that $\rank_{\text{col}}(A\transp) = \rank_{\text{row}}(A)$ by \cref{def:row_and_column_rank}.
\end{enumerate}
\end{proof}
...
basis of it; the passage from $\rank_{\text{col}}(A\transp)$ to
$\rank_{\text{row}}(A)$ is the definition of the two ranks; and

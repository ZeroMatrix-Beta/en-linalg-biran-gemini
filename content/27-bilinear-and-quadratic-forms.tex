...
\begin{enumerate}
    ...
    \item Let $B \in \M_{n \times n}(K)$. Then $B \colon K^n \times K^n \to K$ defined by $B(v, w) := v\transp B w$ is a bilinear form. If $B\transp = B$ (i.e., $B$ is a symmetric matrix), then $B$ is a symmetric bilinear form (left as an exercise). The associated quadratic form is $q(v) = v\transp B v$.
\end{enumerate}
\end{example}
...
\begin{proof}
Choose any basis $\mathcal{C}'$ for $V$ and let $A := [B]_{\mathcal{C}'} \in \M_{n \times n}(\mathbb{R})$. Note that $A$ is a symmetric matrix (since $B$ is symmetric). This implies that $A$ is orthogonally diagonalizable, i.e., there exists $P \in \Orth(n)$ such that $P^{-1} A P$ is a diagonal matrix. But $P^{-1} = P\transp$, so $P\transp A P$ is diagonal.
...
To see why $\eta_1, \dots, \eta_{k+\ell}, 0, \dots, 0$ depend only on $B$ etc., note that if $\mathcal{D}$ and $\mathcal{D}'$ are two orthonormal bases for $V$, then $[\id_V]_{\mathcal{D}'}^{\mathcal{D}} \in \Orth(n)$ (left as an exercise). Put $Q := [\id_V]_{\mathcal{D}'}^{\mathcal{D}}$.
We have
\[
[B]_{\mathcal{D}} = Q\transp \cdot [B]_{\mathcal{D}'} \cdot Q = Q^{-1} [B]_{\mathcal{D}'} Q,
\]
since $Q \in \Orth(n)$. Hence, the matrices $[B]_{\mathcal{D}}$ and $[B]_{\mathcal{D}'}$ are similar, which means their eigenvalues are exactly the same.
\end{proof}
...
\begin{proof}
By the previous theorem (\ref{thm:diagonalization_symmetric_bilinear}), there exists a basis $\mathcal{C}' = (v'_1, \dots, v'_n)$ of $V$ such that
\[
...
\begin{exercisesolution}[Proof that $2 \neq 0 \implies 4 \neq 0$ (on \cpageref{thm:polarization_identity_bilinear})]

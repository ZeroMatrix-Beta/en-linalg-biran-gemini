...
\begin{exercise}[Polarization Identity for Quadratic Forms]
  ...
  \]
  \exhint[Official Hint]{Expand $q_B(v + w) = B(v + w, v + w) = B(v, v) + B(v, w) + B(w, v) + B(w, w)$ using bilinearity. By symmetry, $B(w, v) = B(v, w)$, so $q_B(v + w) = q_B(v) + 2B(v, w) + q_B(w)$. Subtracting $q_B(v) + q_B(w)$ from both sides and dividing by $2$ (which is valid since $\operatorname{char}(K) \neq 2$) gives the result.}

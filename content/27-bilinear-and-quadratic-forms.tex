...
\begin{conclusion}[Quadratic Forms Arise from Symmetric Matrices]
...
\begin{exercise}[Polarization Identity for Quadratic Forms]
  ...
  \]
  \exhint[Hint (Y. St\"aubli)]{Expand $q_B(v + w) = B(v + w, v + w) = B(v, v) + B(v, w) + B(w, v) + B(w, w)$ using bilinearity. By symmetry, $B(w, v) = B(v, w)$, so $q_B(v + w) = q_B(v) + 2B(v, w) + q_B(w)$. Subtracting $q_B(v) + q_B(w)$ from both sides and dividing by $2$ (which is valid since $\operatorname{char}(K) \neq 2$) gives the result.}
...
\defterm{Sylvester's law of inertia}
...
\[
[B]_{\mathcal{C}} = \left( \begin{array}{c|c|c}
  I_k & 0 & 0 \\ \hline
  0 & -I_\ell & 0 \\ \hline
  0 & 0 & 0
\end{array} \right),
...
n - (k + \ell) &= \dim \{ z \in V \mid B(z, v) = 0 \text{ for all } v \in V \}.
\end{align*}
\end{theorem}
...
\[
n - r = \dim \{ w \in V \mid B(w, v) = 0 \text{ for all } v \in V \}.
\]
...
% Extractor: Opus 5
The minors in \textbf{(b)} are the \emph{leading} ones: writing $b_{jk} := B(e_j, e_k)$, the $\ell$-th of them is the determinant of the top-left $\ell \times \ell$ corner of $[B]_{\mathcal{C}}$. Prof.~Biran draws them as a nest of squares growing out of the entry $b_{11}$, which is also the order in which the induction below consumes them:
...
\begin{tikzpicture}[baseline=(M.center)]
  \matrix (M) [matrix of math nodes,
               ...
               nodes={minimum width=2.9em, minimum height=1.8em, inner sep=1pt},
  ...
  % Leading principal minors of orders 1, 2, 3 and n, each drawn with a
  % slightly smaller padding so that the shared top-left corner stays legible.
  \foreach \last/\pad in {1/5, 2/3.5, 3/2, 5/0.5}
    \draw[ThemeAccent, thick]
      ([xshift=-\pad pt, yshift=\pad pt]M-1-1.north west) rectangle
      ([xshift=\pad pt, yshift=-\pad pt]M-\last-\last.south east);
\end{tikzpicture}
\end{center}
% Extractor: Gemini 3.1 Pro
...
\begin{exercise}[Forward Implication of Sylvester's Criterion (\texorpdfstring{\cref{thm:sylvesters_criterion}}{Theorem~\ref{thm:sylvesters_criterion}})]
...
\begin{exercise}[Testing Positive Definiteness via Sylvester's Criterion]
  ...
  \]
  \exhint[Official Hint]{Apply Sylvester's Criterion (\cref{thm:sylvesters_criterion}): a real symmetric matrix is positive definite if and only if all its leading principal minors $\Delta_k := \det(M_{1 \le i,j \le k})$ are strictly positive for $1 \le k \le n$.
...
\begin{exercise}[Equivalent Characterizations of Positive Definiteness \faHeart]
  ...
  \end{enumerate}
  \exhint[Hint (Y. St\"aubli)]{Prove (A) $\implies$ (B) $\implies$ (C) $\implies$ (A).
...
\input{content/27-bilinear-and-quadratic-forms-solutions.tex}
% Creator: Opus 5
\begin{ainote}
The chapter turns on a distinction that is easy to lose. A change of basis acts
on a bilinear form by \emph{congruence},
$[B]_{\mathcal{C}'} = S\transp [B]_{\mathcal{C}} S$
(\cref{lem:change_of_basis_bilinear}), not by similarity. Congruence preserves
the rank and the signature but not the eigenvalues, so the step in
\cref{thm:sylvesters_euclidean} that reads \qt{similar, hence the same
eigenvalues} is doing real work: it is available only because $S$ is taken
orthogonal there, and $S\transp = S^{-1}$ is what turns the congruence into a
similarity. That is why the theorem insists on orthonormal bases, and it is also
why Sylvester's law of inertia and the spectral theorem remain different
statements rather than two readings of one.

Two supporting facts are worth having in the open.
\Cref{lem:matrix_rep_bilinear_form} produces a matrix representation and
thereafter speaks of \emph{the} matrix representation; it is uniqueness that
entitles every later argument to read $([B]_{\mathcal{C}})_{jk} = B(v_j, v_k)$
off the name alone, as the first line of \cref{lem:change_of_basis_bilinear}
does. And the Remark before the Conclusion observes that
$\frac{1}{2}(B + B\transp)$ carries the same quadratic form as $B$; the point of
the observation is that this matrix is \emph{symmetric}, which is exactly what
the Conclusion goes on to need.
\end{ainote}

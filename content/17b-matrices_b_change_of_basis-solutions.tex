% Official Solution (Problem Sheet 13, Exercise 4)
...
\end{enumerate}
\exinfo{Official Solution (Problem Sheet 13, Exercise 4). Proves similarity of $AB$ and $BA$ when at least one matrix is invertible and constructs a nilpotency counterexample otherwise.}
\end{exercisesolution}

% Official Solution (Problem Sheet 13, Multiple Choice Question 2)
...
\begin{itemize}
    ...
    \]
    Thus statement \textbf{(a)} ($r = s$) is true, and statements \textbf{(b)}, \textbf{(c)}, \textbf{(d)} are false.
    ...
    (A - I_n) v = 0 \iff Av = v.
    \]
    Thus statement \textbf{(e)} is true.
\end{itemize}
The true statements are \textbf{(a)} and \textbf{(e)}.
\exinfo{Official Solution (Problem Sheet 13, Multiple Choice Question 2). Shows similarity preserves shifted rank $\rank(A - I_n)$ and connects nullity to fixed vectors.}
\end{exercisesolution}

% Official Solution (Problem Sheet 13, Multiple Choice Question 1)
...
\]
\exinfo{Official Solution (Problem Sheet 13, Multiple Choice Question 1). Computes the change of basis transition matrix between quadratic polynomial bases.}
\end{exercisesolution}

% Official Solution (Problem Sheet 13, Single Choice Question 1)
\begin{exercisesolution}[Solution to \cref{exc:linear_maps_and_base_changes_sc}]
\label{sol:linear_maps_and_base_changes_sc}
Statement \textbf{(c)} is \textbf{false} in general.
...
Statements \textbf{(a)}, \textbf{(b)}, and \textbf{(d)} are all standard true results:
\begin{itemize}
    \item \textbf{(a)} True by definition of change of basis matrix (\cref{def:change_of_basis_matrix}).
    \item \textbf{(b)} True by \cref{cor:vs_isomorphic_to_kn}.
    \item \textbf{(d)} True by \cref{prop:invertibility_isomorphism_equivalence}.
\end{itemize}
\exinfo{Official Solution (Problem Sheet 13, Single Choice Question 1). Evaluates statements on rank upper bounds, change-of-basis definitions, and matrix invertibility.}
\end{exercisesolution}

...
\begin{ainote}
Two of the four results in this section are left to the reader in the notes,
with hints, and both are written out above; \cref{fig:rref_pivot_staircase}
follows the staircase Prof. Biran draws by hand.

One of the two wants more than its hint supplies. The uniqueness criterion does
not drop out of \cref{prop:rank_solvability_criterion} on its own; it needs that
criterion together with the dimension count $\dim \Sol(A, 0) = n - \rank(A)$
from the opening of the section. There is a second place worth slowing down for.
The middle link in the chain of equivalences of
\cref{prop:rank_solvability_criterion} turns on the fact that two nested
subspaces of equal finite dimension coincide; without it, knowing that the two
spans have the same dimension would not put $b$ in the smaller one.

The map $\Phi$, which sends a choice of values for the free variables to the
solution it determines, is introduced in the notes in running prose, with a
numbered statement about it standing directly underneath. It is given a
definition of its own here, left unnumbered so that 17.d.1--17.d.4 keep Prof.
Biran's numbers, and \cref{prop:solution_map_isomorphism} refers to it.
\end{ainote}

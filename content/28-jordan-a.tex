...
\begin{itemize}
    ...
    \item \textbf{Minimal polynomial:} $\mu_A(x) = (x - \lambda_1)^{\ell_1} \cdot (x - \lambda_2)^{\ell_2} \cdot (x - \lambda_3)^{\ell_3}$, where $1 \le \ell_i \le m_a(\lambda_i)$. We will see that $\ell_i$ is precisely the size of the \emph{biggest} block with eigenvalue $\lambda_i$. Thus,
    \[
...
\leavevmode
\begin{enumerate}[label=\textbf{(\alph*)}]
    \item This can be shown by direct calculation (exercise). Alternatively, denote by $e_1, \dots, e_n$ the standard basis for $K^n$. Then the linear map $T_N \colon K^n \to K^n$ acts as a shift operator:
    \[
...
\textbf{Minimal Polynomial:} Let $A := \left(\begin{smallmatrix} B & 0 \\ 0 & C \end{smallmatrix}\right)$ as above. We have $A^k = \left(\begin{smallmatrix} B^k & 0 \\ 0 & C^k \end{smallmatrix}\right)$ (check!). This implies that for every polynomial $q(x) \in K[x]$, we have $q(A) = \left(\begin{smallmatrix} q(B) & 0 \\ 0 & q(C) \end{smallmatrix}\right)$.

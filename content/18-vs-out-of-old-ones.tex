...
\end{remark}
\exhint{Classically, prove the two directions separately: show first that if $T$ is linear, then $\Gamma(T)$ is closed under addition and scalar multiplication; then show that if $\Gamma(T)$ is a subspace, $T$ preserves addition and scalar multiplication.}
...
\end{enumerate}
\exhint{(a) Note that $[v] \in \ker(\bar{f}) \iff f(v) = 0 \iff v \in \ker(f)$. (b) Use (a) and the fact that for a subspace $X \subseteq Y$, $Y/X = \{0\} \iff Y = X$. (c), (d) Combine (b) and the definition of $\Img(\bar{f})$.}
...
\exhint{Recall that $V/U \cong U'$ for any complement $U'$ of $U$. Find which standard basis vectors can be selected to form a complement of $U$ in $\mathbb{R}^5$.}

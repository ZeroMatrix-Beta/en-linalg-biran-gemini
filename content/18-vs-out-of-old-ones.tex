...
\exsol{sol:hom_non_injective_non_surjective_not_subspaces}
\end{exercise}
...
\exsol{sol:dim_kernel_linear_functional_sc}
\end{exercise}
...
\begin{exercise}[Duals of Compositions, of the Identity and of the Zero Map]
  ...
  \end{enumerate}
\exinfo{This exercise is taken from Prof.~Biran's handwritten lecture notes.}
\exsol{sol:dual_functoriality}
\end{exercise}

% Extractor: Gemini 3.1 Pro

\begin{summary}[Dual Spaces as a Contravariant Functor]
...
\exsol{sol:dual_map_equivalences_sc}
\end{exercise}
...
\begin{enumerate}
    ...
    \item Let $\Tr \in \Hom(V, V)^*$ be the trace functional defined by $\Tr(T) := \Tr([T]_{\mathcal{B}}^{\mathcal{B}}) = \sum_{i=1}^n ([T]_{\mathcal{B}}^{\mathcal{B}})_{ii}$ for any chosen basis $\mathcal{B}$ of $V$.
    Show that the linear map
    \[
    \Theta \colon \Hom(V, V) \longrightarrow \Hom(V, V)^*, \qquad T \longmapsto \left( S \mapsto \Tr(S \circ T) \right)
    \]
    is an isomorphism of vector spaces.
\end{enumerate}
\exinfo{This exercise is Problem 3 of Problem Sheet 14, Linear Algebra I, Autumn Semester 2025.}
\exsol{sol:hom_dual_isomorphism_trace_pairing}
\end{exercise}
% Extractor: Gemini 3.1 Pro
...
\exsol{sol:every_finite_dim_is_dual_sc}
\end{exercise}
...
\exinfo{This exercise is taken from Prof.~Biran's handwritten lecture notes.}
\exsol{sol:direct_sum_axioms}
\end{exercise}

\begin{exercise}[Graph of a Linear Map]
\label{exc:graph_of_linear_map}
Let $V$ and $W$ be vector spaces over a field $K$, and let $T \colon V \to W$ be a function. We define the \newterm{graph} of $T$, denoted by $\Gamma(T)$, as the subset of the direct sum $V \oplus W$ given by
\[
\Gamma(T) := \{(v, T(v)) \in V \oplus W \mid v \in V\}.
\]
Prove that $T$ is a linear map if and only if its graph $\Gamma(T)$ is a linear subspace of $V \oplus W$.
\begin{remark}
Formally, a function $T \colon V \to W$ can be viewed as a subset $T \subseteq V \times W = V \oplus W$ such that for each $v \in V$ there exists a unique $w \in W$ with $(v, w) \in T$. Thus, a function from $V$ to $W$ is linear if and only if it is a linear subspace of $V \oplus W$.
\end{remark}
\exhint[Hint (Y. St\"aubli)]{Classically, prove the two directions separately: show first that if $T$ is linear, then $\Gamma(T)$ is closed under addition and scalar multiplication; then show that if $\Gamma(T)$ is a subspace, $T$ preserves addition and scalar multiplication.}
\exinfo{This exercise is Problem 3 of Problem Sheet 15, Linear Algebra II, Spring Semester 2026.}
\exsol{sol:graph_of_linear_map}
\end{exercise}
...
\begin{exercise}[Subspaces, Complements, and Dual Spaces \faHeart]
\label{exc:subspace_complement_dual_decomposition}
Let $V$ be a finite-dimensional vector space over a field $K$.
\begin{enumerate}
    \item Let $U \subseteq V$ be a linear subspace and $W \subseteq V$ a linear complement ($V = U \oplus W$). Define an explicit isomorphism between $V$ and the external direct sum $U \oplus W$.
    \item Show that every linear functional $\alpha \colon U \to K$ can be extended to a linear functional $\tilde{\alpha} \colon V \to K$ (i.e., $\tilde{\alpha}|_U = \alpha$). Does $\tilde{\alpha}$ depend on the choice of the complement $W$ of $U$?
    \item Construct a canonical isomorphism
    \[
    V^* \cong U^* \oplus W^*.
    \]
\end{enumerate}
\exhint[Hint (Y. St\"aubli)]{(a) Recall the canonical map $(u, w) \mapsto u + w$ from the lecture. (b) Every linear map is uniquely determined by its values on a basis; extend a basis of $U$ to $V$ using a basis of $W$. (c) For $\ell \in V^*$, consider the restricted functionals $\ell|_U = \ell \circ \iota_U \in U^*$ and $\ell|_W = \ell \circ \iota_W \in W^*$. Use these to define a linear map $\psi \colon V^* \to U^* \oplus W^*$ and prove that it is bijective.}
\exinfo{This exercise is Problem 4 of Problem Sheet 15, Linear Algebra II, Spring Semester 2026.}
\exsol{sol:subspace_complement_dual_decomposition}
\end{exercise}

\begin{exercise}[Direct Sum Decompositions of Homomorphism Spaces \faHeart]
\label{exc:hom_direct_sum_decompositions}
Let $U, V, W_1, W_2$ be finite-dimensional vector spaces over a field $K$.
\begin{enumerate}
    \item Construct an explicit natural isomorphism
    \[
    \Hom(V, W_1 \oplus W_2) \cong \Hom(V, W_1) \oplus \Hom(V, W_2).
    \]
    \item Construct an explicit natural isomorphism
    \[
    \Hom(V \oplus U, W_1) \cong \Hom(V, W_1) \oplus \Hom(U, W_1).
    \]
\end{enumerate}
\exhint[Hint (Y. St\"aubli)]{While dimension-counting shows the spaces are isomorphic, the goal is to construct explicit coordinate-free isomorphisms: (a) For $f \in \Hom(V, W_1 \oplus W_2)$, consider the compositions $\pi_{W_1} \circ f$ and $\pi_{W_2} \circ f$ with the canonical projections, and define $\Phi(f) := (\pi_{W_1} \circ f, \pi_{W_2} \circ f)$; exhibit its inverse explicitly. (b) For $f \in \Hom(V \oplus U, W_1)$, consider the precompositions $f \circ \iota_V$ and $f \circ \iota_U$ with the canonical inclusions.}
\exinfo{This exercise is Problem 6 (parts a and b) of Problem Sheet 15, Linear Algebra II, Spring Semester 2026.}
\exsol{sol:hom_direct_sum_decompositions}
\end{exercise}
...
\defterm{first isomorphism theorem}
...
% thm:18.c.6
  ...
  Let $V$ be a vector space over $K$, and let $U \subseteq V$ be a subspace. The quotient space $V/U$ has the following universal property: For every vector space $W$ and every linear map $T \colon V \to W$ for which $T(U) = 0$ (i.e., for which $\ker(T) \supseteq U$), there exists a unique linear map $T' \colon V/U \to W$ such that $T = T' \circ \pi$, that is, such that the following triangle commutes:
  \begin{center}
...
\begin{proof}
  ...
  \begin{exercise}[Well-definedness and Linearity of the Induced Map]
    ...
    \end{enumerate}
  \exinfo{This exercise is taken from Prof.~Biran's handwritten lecture notes.}
  \exsol{sol:induced_map_well_defined}
\end{exercise}
...
\exinfo{This exercise is taken from Prof.~Biran's handwritten lecture notes.}
\exsol{sol:kernel_induced_map}
\end{exercise}

\begin{exercise}[Properties of Induced Maps on Quotient Spaces \faHeart]
\label{exc:properties_induced_quotient_maps}
Let $V$ and $W$ be vector spaces over a field $K$, let $U \subseteq V$ be a linear subspace, and let $f \colon V \to W$ be a linear map such that $U \subseteq \ker(f)$. Consider the induced linear map
\[
\bar{f} \colon V/U \longrightarrow W, \qquad v + U \longmapsto f(v),
\]
guaranteed by the universal property of quotient spaces. Show that:
\begin{enumerate}
    \item $\ker(\bar{f}) = \ker(f)/U$.
    \item $\bar{f}$ is injective if and only if $U = \ker(f)$.
    \item $\bar{f}$ is surjective if and only if $f$ is surjective.
    \item If $f$ is surjective, then $\bar{f}$ induces a canonical isomorphism $V/\ker(f) \cong W$.
\end{enumerate}
\exhint[Hint (Y. St\"aubli)]{(a) Note that $[v] \in \ker(\bar{f}) \iff f(v) = 0 \iff v \in \ker(f)$. (b) Use (a) and the fact that for a subspace $X \subseteq Y$, $Y/X = \{0\} \iff Y = X$. (c), (d) Combine (b) and the definition of $\Img(\bar{f})$.}
\exinfo{This exercise is Problem 2 of Problem Sheet 15, Linear Algebra II, Spring Semester 2026.}
\exsol{sol:properties_induced_quotient_maps}
\end{exercise}
...
\exinfo{This exercise is taken from Prof.~Biran's handwritten lecture notes.}
\exsol{sol:inverse_from_complement}
\end{exercise}

\begin{exercise}[Basis of a Quotient Space]
\label{exc:basis_of_quotient_space_r5}
Consider the linear subspace
\[
U := \operatorname{span}\left( \begin{pmatrix} 2 \\ 2 \\ 2 \\ 2 \\ 2 \end{pmatrix}, \begin{pmatrix} 1 \\ 2 \\ 2 \\ 2 \\ 2 \end{pmatrix}, \begin{pmatrix} 1 \\ 1 \\ 2 \\ 2 \\ 2 \end{pmatrix} \right) \subseteq V := \mathbb{R}^5.
\]
Determine a subset of the standard basis $(e_1, e_2, e_3, e_4, e_5)$ of $\mathbb{R}^5$ whose image under the canonical quotient map $\pi \colon V \to V/U$ forms a basis of the quotient space $V/U$.
\exhint[Hint (Y. St\"aubli)]{Recall that $V/U \cong U'$ for any complement $U'$ of $U$. Find which standard basis vectors can be selected to form a complement of $U$ in $\mathbb{R}^5$.}
\exinfo{This exercise is Problem 1 of Problem Sheet 15, Linear Algebra II, Spring Semester 2026.}
\exsol{sol:basis_of_quotient_space_r5}
\end{exercise}
...
\begin{exercise}[Relationship between Annihilator of Image and Kernel of Dual Map]
  ...
  \[ (\Img T)^\perp = \ker(T^*). \]
\exinfo{This exercise is taken from Prof.~Biran's handwritten lecture notes.}
\exsol{sol:annihilator_image_kernel_dual}
\end{exercise}
...
\label{sol:induced_map_well_defined}
\begin{enumerate}[label=\textbf{(\alph*)}]
...
\label{sol:inverse_from_complement}
...
\begin{enumerate}[label=\textbf{(\alph*)}]
    ...
    \begin{align*}
    L(\alpha S + S')(v) &= \big((\alpha S + S') \circ \pi\big)(v) = (\alpha S + S')\big(\pi(v)\big) \\
    &= \alpha \, S(\pi(v)) + S'(\pi(v)) = \big(\alpha L(S) + L(S')\big)(v),
    \end{align*}
...
\label{sol:annihilator_image_kernel_dual}
...
\input{content/18-vs-out-of-old-ones-solutions.tex}

\begin{ainote}
Three lectures stand behind this chapter: $\Hom$, duals and biduals in 18.a,
direct sums in 18.b, quotients in 18.c. They share one move. Each takes vector
spaces already in hand and manufactures a new one from them, and in each case
the work is to check that what comes out is again a vector space and to find its
dimension. The dual is the case to watch, because $V^*$ and $V$ have the same
dimension without there being any canonical isomorphism between them, while $V$
and $V^{**}$ do have one. \Cref{thm:natural_map_injective} and the naturality of
$\tau$ on \cpageref{prop:naturality_of_tau} are where that distinction is made
precise, and it is worth reading them together: the first says the map exists
and is injective, the second says it does not depend on any choice.

The notes leave two things to the lecture rather than to the page. Where they
say only \qt{explain briefly the structure of a ring} and \qt{explain what is a
homomorphism of rings}, both given orally, this book has to write something
down, since it uses the word from \cpageref{rem:matrix_ring} onwards and states
\cref{cor:endomorphism_ring} as an isomorphism of rings. \Cref{aidef:ring} and
\cref{aidef:ring_homomorphism} fill that gap, and are marked as not being Prof.
Biran's. The notes also mark fourteen results \qt{Exc.}, from the vector space
axioms for $\Hom(V,W)$ to $(\Img T)^\perp = \ker(T^*)$; all fourteen are
exercises here, and each of the three sections closes with their solutions.
\end{ainote}

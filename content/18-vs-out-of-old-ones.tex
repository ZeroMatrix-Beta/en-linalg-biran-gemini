...
\begin{proof}
  ...
  \]
  This means $\Hom(V,W)$ is a finite-dimensional space. Furthermore,
  \[
...
\begin{proof}
  ...
  Here, only $k = j$ remains from the inner sum. This implies that $[\id_{V^*}]_{\mathcal{B}^*}^{\mathcal{C}^*} = A\transp = ([\id_V]_{\mathcal{C}}^{\mathcal{B}})\transp$.
\end{proof}
...
\begin{exercise}[Duals of Identity and Zero Maps]
...
\subsection{The Double Dual (or Bidual) Space}

Let $V$ be a vector space over $K$. We have $V^*$, but also $V^{**} := (V^*)^*$, obtained by applying the \newterm{dual} operation one more time to $V^*$. $V^{**}$ is sometimes called the \newterm{bidual space} of $V$. Is there any relation between $V^{**}$ and $V$?
\end{summary}
...
Let $V$ be a vector space over $K$, and let $U \subseteq V$ be a subspace. We will define a new vector space over $K$, denoted by $V/U$, together with a linear map $\pi \colon V \to V/U$ such that $\pi$ is surjective and $\ker(\pi) = U$. The space $V/U$ will have some other good properties.

\begin{definition*}[The Quotient $V/U$ And The Quotient Map $\pi$] The space $V/U$ is called the \newterm{quotient} of $V$ by $U$, or $V$ modulo $U$. The map $\pi$ is called the \newterm{quotient map}, or the \newterm{projection onto the quotient space}.
\end{definition*}
...
\begin{definition}[The Quotient Space]
  Let $V$ be a vector space over $K$, and let $U \subseteq V$ be a subspace. We define $V/U := V/\sim$ to be the set of equivalence classes of the equivalence relation $\sim$. So, the elements of $V/U$ are $[v]$, where $v \in V$. 
...
\begin{itemize}
  ...
  \item If $U$ is a \textbf{line} through the origin, the equivalence classes $[v]$ forming the space $\mathbb{R}^2/U$ are the lines parallel to $U$, as seen in Figure \cref{fig:quotient_r2}.
\end{itemize}
...
Similarly, we can extend this intuition to $\mathbb{R}^3$, as illustrated in Figure \cref{fig:quotient_r3}:
\begin{itemize}
  ...
  \item If $U$ is a \textbf{plane} through the origin, then $\mathbb{R}^3/U$ is 1-dimensional (by \cref{prop:dim_quotient}) and therefore isomorphic to a line. Each element $[v]$ is a plane parallel to $U$. The quotient space can be viewed as the set of all such parallel planes \qt{stacked} along a transversal direction (see Figure \cref{fig:quotient_plane} and \cref{prop:iso_complement}).
  \item If $U$ is a \textbf{line} through the origin, then $\mathbb{R}^3/U$ is 2-dimensional (by \cref{prop:dim_quotient}) and therefore isomorphic to a plane. Each element $[v]$ is a line parallel to $U$ (see Figure \cref{fig:quotient_line} and \cref{prop:iso_complement}).
...
\begin{proof}
  ...
  Since $\ker(\pi) = U$ and $\Img(\pi) = V/U$, we obtain $\dim V/U = \dim V - \dim U$.
\end{proof}
...
\begin{proof}
  \begin{enumerate}
    ...
    \item The commutativity of the diagram follows immediately from the definitions. (Exercise: check this!)
  \end{enumerate}
\end{proof}
...
\begin{exercise}[Inverse Isomorphism from a Complement]
  Describe the inverse of $Q$, $Q^{-1} \colon V/U \xrightarrow{\cong} W$. (Hint: \dots)
\end{exercise}
...
\begin{proof}
  \begin{enumerate}
    ...
    We claim that $L$ is an isomorphism. To see this, we define a map $P \colon U^\perp \to (V/U)^*$ as follows. Let $T \in U^\perp$, i.e., $T \colon V \to K$ such that $T|_U = 0$. By the Universal Property of Quotients, there exists a unique $T' \colon V/U \to K$ such that $T = T' \circ \pi$. Define $P(T) := T'$.
    ...
    We claim that $R$ is surjective. Indeed, let $\psi \in U^*$. We need to show that there exists $\varphi \in V^*$ such that $\varphi|_U = \psi$. The finite-dimensionality of $V$ is important here. Pick a basis $u_1, \dots, u_k$ for $U$, and extend it to a basis $u_1, \dots, u_k, v_{k+1}, \dots, v_n$ of $V$. Define a linear map $\varphi \colon V \to K$ by $\varphi(u_i) := \psi(u_i)$ for all $1 \leq i \leq k$, and $\varphi(v_j) := 0$ for all $k < j \leq n$. Clearly, $\varphi|_U = \psi$ because $\varphi$ and $\psi$ coincide on the elements of a basis of $U$. This proves $\Img(R) = U^*$.

    We claim that $\ker(R) = U^\perp$. Indeed, this follows directly from definitions: $\ker(R) = \{\varphi \in V^* \mid \varphi|_U = 0\} = U^\perp$. 
    By the First Isomorphism Theorem, $R$ induces an isomorphism $\overline{R} \colon V^*/U^\perp \xrightarrow{\cong} U^*$.
  \end{enumerate}
\end{proof}

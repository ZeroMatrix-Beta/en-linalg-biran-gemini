...
\begin{proof}
  ...
  Regarding the neutral element, the map $\mathcal{O} \colon V \to W$ defined by $v \mapsto 0$ for all $v \in V$ is linear. We claim that $\mathcal{O} + T = T$. Indeed,
  $$(\mathcal{O} + T)(v) = \mathcal{O}(v) + T(v) = 0 + T(v) = T(v)$$
  for all $v \in V$. Similarly, one shows that $T + \mathcal{O} = T$.
\end{proof}

\begin{exercise}
  Complete the proof that $\Hom(V,W)$ is a vector space by checking that it satisfies all the axioms of a vector space.
\end{exercise}
...
\begin{exercise}
  ...
  \begin{enumerate}
    \item $(\id_V)^* = \id_{V^*}$.
    \item $(\mathcal{O} \colon V \to V)^* = (\mathcal{O} \colon V^* \to V^*)$.
  \end{enumerate}
\end{exercise}

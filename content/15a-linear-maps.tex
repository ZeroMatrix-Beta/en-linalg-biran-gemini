...
\end{equation}
Because the coordinates $a_i$ are uniquely determined for each $v$, the map $T$ is well-defined. By verifying the conditions of \cref{def:linear_map}, it is straightforward to show that $T$ is linear. Furthermore, for each basis vector $v_j$, the only non-zero coordinate is $a_j = 1$, which naturally implies $T(v_j) = w_j$.
...
\end{pmatrix}.
\]
The linear transformation $T_A: K^n \to K^m$ defined by $v \mapsto A \cdot v$ easily satisfies $T_A(e_j) = w_j$ for all $j$. Due to the absolute uniqueness property established in \cref{thm:existence_uniqueness_linear_map}, we immediately conclude that $T = T_A$.
\end{answer}
\end{example}
...
\begin{proof}
By the Rank-Nullity Theorem \eqref{thm:rank_nullity}, we know that $\dim \ker(T) = \dim V - \dim \Img(T)$. Since we are given that $\dim V = \dim W$, we can analyze the implications:
\begin{itemize}
    \item $T$ is injective $\iff \ker(T) = \{0_V\} \iff \dim \ker(T) = 0 \iff \dim \Img(T) = \dim V = \dim W \iff T$ is surjective.
\end{itemize}

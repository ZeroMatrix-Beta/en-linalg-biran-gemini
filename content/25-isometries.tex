...
\textbf{(b)} $\implies$ \textbf{(a)}: Using the polarization identity for the real case (on \cpageref{exc:reconstruct_scalar_prod}), we have:
\begin{align*}
...
The remaining implications, in particular \textbf{(d)} $\implies$ \textbf{(e)}, are established in the exercise solutions at the end of this chapter.
\end{proof}
...
Now, $T_A|_L \colon L \to L$ is a linear isometry. Let $\mathcal{L} = (w_1, w_2)$ be an orthonormal basis for $L$. By \cref{prop:orth_unitary_matrix_rep}, the representation matrix is $Q := [T_A|_L]_{\mathcal{L}}^{\mathcal{L}} \in \Orth(2)$.
...
Assume there exists an orthonormal basis $\mathcal{B} = (e_1, \dots, e_n)$ of $V$ such that $Te_1, \dots, Te_n$ is also an orthonormal basis. Let $1 \leq j \leq n$. Then, for all $1 \leq i \leq n$, the defining property \eqref{eq:adjoint_property} of the adjoint map gives
\begin{equation}

...
\begin{enumerate}
    ...
    \[ \operatorname{dist}_W(Tp, Tq) = \|Tp - Tq\|_W = \|p - q\|_V = \operatorname{dist}_V(p, q). \]
...
\begin{enumerate}
    ...
    \item There exists an orthonormal \underline{basis} $\mathcal{B} = (e_1, \dots, e_n)$ of $V$ such that $Te_1, \dots, Te_n$ is also an orthonormal basis.
    \item $T \circ T^* = \id_V$.
    \item $T^* \circ T = \id_V$.
    \item $T^* = T^{-1}$.
    \item $T^*$ is an isometry.
...
\begin{itemize}
    \item $A \in \M_{n \times n}(\mathbb{R})$ is orthogonal if $A^T A = I \iff AA^T = I \iff A^{-1} = A^T$. The set of such matrices is $\operatorname{O}(n)$.
    \item $A \in \M_{n \times n}(\mathbb{C})$ is unitary if $A^* A = I \iff AA^* = I \iff A^{-1} = A^*$. The set of such matrices is $\operatorname{U}(n)$.
\end{itemize}

% Prop. (25.a.3)
\begin{proposition}
\label{prop:orth_unitary_matrix_rep}
Let $V$ be an $n$-di\-men\-sional inner product space over $\mathbb{R}$ (resp. $\mathbb{C}$). Let $\mathcal{B}$ be an orthonormal basis for $V$, and let $T \in \End(V)$. Then, $T$ is orthogonal (resp. unitary) if and only if $[T]_{\mathcal{B}}^{\mathcal{B}} \in \operatorname{O}(n)$ (resp. $[T]_{\mathcal{B}}^{\mathcal{B}} \in \operatorname{U}(n)$).
\end{proposition}
...
Note that for all $A \in \operatorname{O}(n)$, we have $\det(A) = \pm 1$. Indeed:
\[ 1 = \det(I) = \det(AA^T) = \det(A) \cdot \det(A^T) = \det(A) \cdot \det(A) = \det(A)^2. \]

Similarly, for all $A \in \operatorname{U}(n)$, we have $|\det(A)| = 1$. Indeed:
...
\begin{align*}
    \operatorname{SO}(n) &:= \{ A \in \operatorname{O}(n) \mid \det(A) = 1 \} \subseteq \operatorname{O}(n) \subseteq \GL_n(\mathbb{R}), \\
    \operatorname{SU}(n) &:= \{ A \in \operatorname{U}(n) \mid \det(A) = 1 \} \subseteq \operatorname{U}(n) \subseteq \GL_n(\mathbb{C}).
\end{align*}
\end{definition}
...
Let $G$ be any of the sets $\operatorname{O}(n), \operatorname{SO}(n), \operatorname{U}(n)$, or $\operatorname{SU}(n)$. Then:
\begin{enumerate}
    \item $I_n \in G$.
...
Let $A \in \operatorname{O}(n)$. If $\lambda$ is a real eigenvalue of $A$, then $\lambda = \pm 1$.
\end{lemma}
...
% Prop. (25.a.7)
\begin{proposition}[Classification of $\operatorname{O}(2)$]
\label{prop:classification_o2}
Let $A \in \operatorname{O}(2)$.
\begin{enumerate}
...
Now, let $A = \begin{pmatrix} | & | \\ u_1 & u_2 \\ | & | \end{pmatrix} \in \operatorname{O}(2)$. 
...
If $u_2 = u_2'$, then $A = \begin{pmatrix} \cos\theta & -\sin\theta \\ \sin\theta & \cos\theta \end{pmatrix} = R_\theta$, and $\det A = 1$.

If $u_2 = u_2''$, then $A = \begin{pmatrix} \cos\theta & \sin\theta \\ \sin\theta & -\cos\theta \end{pmatrix} = R_\theta \cdot \begin{pmatrix} 1 & 0 \\ 0 & -1 \end{pmatrix}$, and $\det A = -1$.
...
\begin{summary}
$\operatorname{SO}(2) =$ rotations. \\
$\operatorname{O}(2) \setminus \operatorname{SO}(2) =$ reflections.
\end{summary}
...
% Prop. (25.a.9)
\begin{proposition}[Classification of $\operatorname{SO}(3)$]
\label{prop:classification_so3}
Let $A \in \operatorname{SO}(3)$. Then $1$ is an eigenvalue of $A$. 
...
\begin{enumerate}
    ...
    \item If $-1$ is \underline{not} an eigenvalue of $A$ (which is true if and only if $1$ is the only real eigenvalue), then $\theta \neq \pi$.
\end{enumerate}
\end{proposition}
...
By a previous theorem (\cref{lem:eigenvalues_orthogonal}), $\lambda = \pm 1$.
...
Indeed, if $v \in L$, this implies $0 = \langle v, v_1 \rangle$. Because $A \in \operatorname{O}(3)$, it preserves inner products:
...
Now, $T_A|_L \colon L \to L$ is a linear isometry. Let $\mathcal{L} = (w_1, w_2)$ be an orthonormal basis for $L$. By a previous result, the representation matrix is $Q := [T_A|_L]_{\mathcal{L}}^{\mathcal{L}} \in \operatorname{O}(2)$.
...
So, $Q \in \operatorname{SO}(2)$. By the previous proposition, there exists $\theta \in [0, 2\pi)$ such that $Q = R_\theta$.
Thus,
...
% Prop. (25.a.10)
\begin{proposition}[Classification of $\operatorname{O}(3) \setminus \operatorname{SO}(3)$]
\label{prop:classification_o3_minus_so3}
Let $A \in \operatorname{O}(3) \setminus \operatorname{SO}(3)$. Then there exists an orthonormal basis $\mathcal{B} = (v_1, v_2, v_3)$ for $\mathbb{R}^3$ such that 
\[ [T_A]_{\mathcal{B}}^{\mathcal{B}} = \begin{pmatrix} -1 & 0 & 0 \\ 0 & \cos\theta & -\sin\theta \\ 0 & \sin\theta & \cos\theta \end{pmatrix} = \begin{pmatrix} -1 & 0 \\ 0 & R_\theta \end{pmatrix} \]
for some $\theta \in [0, 2\pi)$. (So $T_A$ does a rotation by $\theta$ around $\Sp(v_1)$, followed by a reflection with respect to $\Sp(v_2, v_3)$).
\end{proposition}

\begin{proof}
Let $A' := -A = (-I) \cdot A$. Then $A' \in \operatorname{O}(3)$ and $\det A' = (-1)^3 \cdot \det A = (-1) \cdot (-1) = 1$.
This implies $A' \in \operatorname{SO}(3)$. By the previous proposition, there exists an orthonormal basis $\mathcal{B}$ for $\mathbb{R}^3$, and $\theta' \in [0, 2\pi)$, such that $[T_{A'}]_{\mathcal{B}}^{\mathcal{B}} = \left(\begin{smallmatrix} 1 & 0 \\ 0 & R_{\theta'} \end{smallmatrix}\right)$. 
This implies:
\begin{align*}
...
We will prove this generically for $G = \operatorname{U}(n)$ and $G = \operatorname{SU}(n)$ (the real cases are identical, replacing the conjugate transpose $^*$ with the transpose $^\top$).
\begin{description}
    \item[Proof of \textbf{(a)}:] The identity matrix $I_n$ satisfies $I_n^* \cdot I_n = I_n \cdot I_n = I_n$, so $I_n \in \operatorname{U}(n)$. Furthermore, $\det(I_n) = 1$, so $I_n \in \operatorname{SU}(n)$.
    
    \item[Proof of \textbf{(b)}:] Let $A, B \in \operatorname{U}(n)$. We evaluate $(AB)^* (AB) = (B^* A^*) (AB) = B^* (A^* A) B$. Since $A \in \operatorname{U}(n)$, $A^* A = I_n$, so this reduces to $B^* I_n B = B^* B$. Since $B \in \operatorname{U}(n)$, $B^* B = I_n$. Thus, $AB \in \operatorname{U}(n)$. 
    If additionally $A, B \in \operatorname{SU}(n)$, then $\det(AB) = \det(A)\det(B) = 1 \cdot 1 = 1$, so $AB \in \operatorname{SU}(n)$.
    
    \item[Proof of \textbf{(c)}:] Let $A \in \operatorname{U}(n)$. This implies $A^* A = I_n$, meaning $A^{-1} = A^*$. We check if $A^{-1}$ is unitary: $(A^{-1})^* A^{-1} = (A^*)^* A^* = A A^* = I_n$. Thus, $A^{-1} \in \operatorname{U}(n)$. 
    If additionally $A \in \operatorname{SU}(n)$, then $\det(A^{-1}) = \frac{1}{\det(A)} = \frac{1}{1} = 1$, so $A^{-1} \in \operatorname{SU}(n)$.
\end{description}
\end{exercisesolution}
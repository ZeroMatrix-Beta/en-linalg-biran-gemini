...
\defterm{change of basis formula}
% cor:17.b.1
...
\begin{enumerate}
    \item Let $A, B \in \M_{m \times n}(K)$. We say that $A$ and $B$ are \newterm[equivalent matrices]{equivalent} if there exist matrices $P \in \GL_m(K)$ and $Q \in \GL_n(K)$ such that $B = P A Q$.
...
\begin{enumerate}[label=\textbf{(\alph*)}]
    ...
    \item Show that two matrices $A, B \in \M_{m \times n}(K)$ are equivalent if and only if there exist two vector spaces $V$ and $W$ with $\dim V = n$ and $\dim W = m$, a linear map $T \colon V \to W$, two bases $\mathcal{B}$ and $\mathcal{B}'$ of $V$, and two bases $\mathcal{C}$ and $\mathcal{C}'$ of $W$ such that:
    \begin{equation*}
...
\exsol{sol:equivalence_similarity_properties}
\end{exercise}
...
\exsol{sol:similarity_of_matrix_products}
\end{exercise}
...
\exsol{sol:rank_invariance_similarity_mc}
\end{exercise}
...
\exsol{sol:change_of_basis_polynomials_mc}
\end{exercise}
...
\exsol{sol:linear_maps_and_base_changes_sc}
\end{exercise}
...
\label{sol:equivalence_similarity_properties}
...
\input{content/17b-matrices_b_change_of_basis-solutions.tex}

\begin{ainote}
The two relations this section introduces are easy to define and easy to
misremember, and \cref{exc:equivalence_similarity_properties} is where they
acquire their meaning. Two matrices are equivalent exactly when they represent
one and the same linear map in two pairs of bases; two square matrices are
similar exactly when they represent one and the same endomorphism in two bases.
Both readings turn on a single observation: for an invertible $P$, the columns
of $P$ form a basis, and $P$ is itself the transition matrix from the standard
basis to that one. Most of what follows is that observation applied twice, once
on each side.

The composition rule repays being used as a check and not only as a formula. In
a product $[S]_{\mathcal{E}}^{\mathcal{C}} \cdot [T]_{\mathcal{C}}^{\mathcal{B}}$
the two inner bases have to agree, and an expression in which they do not fails
to compose at all, whatever its shapes may permit. That safeguard earns its keep
here, because the characteristic error in this material is not arithmetic:
writing $[\id_{K^m}]_{\mathcal{E}_m}^{\mathcal{C}}$ where
$[\id_{K^m}]_{\mathcal{C}}^{\mathcal{E}_m}$ is meant yields the \emph{inverse}
of the matrix intended, and since the inverse of a transition matrix is again a
transition matrix, nothing about the resulting expression looks out of place.
Reading the inner bases off before anything else catches it immediately, and it
is the discipline \cref{cor:matrix_equivalence_normal_form} and
\cref{cor:change_of_basis_formula} both reward.
\end{ainote}

...
\begin{enumerate}
\item The \newterm{column space} $\ColS(A) \subseteq K^{m}$ is the span of the columns of $A$.
\item The \newterm{row space} $\RowS(A) \subseteq K^{n}$ is the span of the rows of $A$.
\end{enumerate}
\end{definition}
...
For any $A \in \M_{m \times n}(K)$, $\dim \RowS(A) = \dim \ColS(A)$. This shared dimension is the \newterm{rank} of $A$, denoted $\rank(A)$.
\end{theorem}
\begin{proof}

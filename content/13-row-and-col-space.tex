...
&\xrightarrow[\; R_{3} - R_{1} \to R_{3} \;]{\; R_{2} - R_{1} \to R_{2} \;}
\left( \begin{array}{cc|c} 1 & 1 & b_{1} \\ 0 & -1 & b_{2}-b_{1} \\ 0 & -2 & b_{3}-b_{1} \end{array} \right) \\[0.5em]
&\xrightarrow{\; -1 \cdot R_{2} \to R_{2} \;}
\left( \begin{array}{cc|c} 1 & 1 & b_{1} \\ 0 & 1 & b_{1}-b_{2} \\ 0 & -2 & b_{3}-b_{1} \end{array} \right) \\[0.5em]
&\xrightarrow{\;R_{3} + 2 \cdot R_{2} \to R_{3} \;}
\left( \begin{array}{cc|c} 1 & 1 & b_{1} \\ 0 & 1 & b_{1}-b_{2} \\ \hline 0 & 0 & b_{1}-2b_{2}+b_{3} \end{array} \right) \\[0.5em]
&\xrightarrow{\; R_{1} - R_{2} \to R_{1} \;}
\left( \begin{array}{cc|c} 1 & 0 & b_{2} \\ 0 & 1 & b_{1}-b_{2} \\ \hline 0 & 0 & b_{1}-2b_{2}+b_{3} \end{array} \right)
\end{align*}
The last step clears the entry above the second pivot, which is what makes the left block row-reduced and not merely echelon; it does not affect the zero row, and it has the pleasant side effect that the solution can now be read off directly, as $x_{1} = b_{2}$ and $x_{2} = b_{1} - b_{2}$.
...
\textbf{Type 3 ($R_{i} \leftrightarrow R_{j}$).} Exchanging two rows of $M$ merely reorders the list $w_{1}, \dots, w_{m}$, and the span of a list does not depend on the order of its members. Hence, $\RowS(M)$ is unchanged by such an operation.

\textbf{Type 1 ($\lambda \cdot R_{i} \to R_{i}$, with $\lambda \in K \setminus \{0\}$).} Replacing $w_{i}$ by $\lambda w_{i}$ does not change the span of the rows either: the new row lies in $\Sp(w_{1}, \dots, w_{m})$, and conversely $w_{i} = \lambda^{-1} (\lambda w_{i})$ lies in the span of the new list, which is where the hypothesis $\lambda \neq 0$ enters. So, $\RowS(M)$ is left unchanged by such an operation as well.

\textbf{Type 2 ($R_{i} + \lambda \cdot R_{j} \to R_{i}$, with $i \neq j$ and $\lambda \in K$).} Here we claim that
\[
...
\label{rem:row_ops_and_columns}
Row operations \textbf{do} change the column space, as seen by $\begin{pmatrix} 1 & 1 \\ 1 & 1 \end{pmatrix} \to \begin{pmatrix} 1 & 1 \\ 0 & 0 \end{pmatrix}$. However, they \textbf{preserve} the linear relations between columns: writing $w_1, \dots, w_n$ for the columns of $A$ and $w'_1, \dots, w'_n$ for the columns of $A'$, we have $\sum \alpha_{i} w_{i} = 0 \iff \sum \alpha_{i} w'_{i} = 0$.
\end{importantremark}

The second assertion of \cref{rem:row_ops_and_columns} carries the entire weight of \cref{thm:basis_col_space} below, which transfers a statement about the echelon form back to the original matrix. The notes do not prove it, so we record it here, together with the two consequences in which it is actually used.
...
\begin{proof}
By \cref{prop:row_space_invariance}, $\RowS(A) = \RowS(B)$. Let $r$ be the number of pivots in $B$. The non-zero rows of $B$ are $u'_1, \dots, u'_r$, the zero rows of a row-reduced echelon matrix standing below them. They span $\RowS(B)$: by \cref{def:matrix_subspaces} that space is spanned by \emph{all} rows of $B$, and discarding the zero ones changes no linear combination.
...
Suppose $\sum_{i=1}^r \alpha_i u'_i = 0$. Looking at the $j_k$-th component of this vector sum, we have:
...
\]
and therefore, every statement about row rank is a statement about the column rank of the transposed matrix, and conversely.
\end{remark}

\begin{ainote}
Two pieces of notation arrive later in this chapter than a reader expects them
to. The transpose is defined only in the closing section
(\cref{def:transpose}), although $A\transp$ is used from here on through
fourteen chapters; and the two ranks are named in
\cref{def:row_and_column_rank}, just before the theorem that identifies them.
Both are worth locating on a first pass, since almost nothing later stops to
recall either.

One step of the argument is not Prof. Biran's and is marked accordingly.
\Cref{thm:basis_col_space}, which says that the columns of $A$ sitting over the
pivots of its row-reduced form are a basis of $\ColS(A)$, rests on the principle
that row operations preserve the linear relations among columns. The notes state
that principle in a remark and use it without proof, and it carries real weight:
it is the entire reason a computation performed on the rows says anything at all
about the columns. It appears here as
\cref{ailem:column_relations_preserved}, proved from
\cref{thm:ero_preserve_solutions}. Prof. Biran also defers
$\rankrow(A) = \rankcol(A)$ to a later lecture, whereas this chapter reaches it
already, in \cref{thm:rank_equality}.

One oddity is left as it stands, deliberately. The rank of a matrix is named
inside the statement of \cref{thm:rank_equality} rather than in a definition of
its own. The notion stays citable, that theorem being numbered, and nothing
later appeals to a definition of matrix rank that does not exist: the two places
in ch. 17c that say \qt{by definition, $\rank(\cdot)$} mean the rank of a linear
map, which \cref{def:rank_linear_map} supplies.
\end{ainote}

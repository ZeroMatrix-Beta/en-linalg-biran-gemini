...
For any $A \in \M_{m \times n}(K)$, $\dim \RowS(A) = \dim \ColS(A)$, or equivalently, in the terminology of \cref{def:row_and_column_rank}, $\rank_{\text{row}}(A) = \rank_{\text{col}}(A)$. This shared dimension is the \newterm{rank} of $A$, denoted $\rank(A)$.
\end{theorem}
\begin{proof}
...
\[
\rank_{\text{row}}(A\transp) = \rank_{\text{col}}(A), \qquad \rank_{\text{col}}(A\transp) = \rank_{\text{row}}(A),
\]
...
defer $\rank_{\text{row}}(A) = \rank_{\text{col}}(A)$ to a later lecture, while

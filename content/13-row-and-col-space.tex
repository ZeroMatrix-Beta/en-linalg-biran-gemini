...
\begin{ainote}
Several pieces of the notes were missing from this transcript and are restored
above, among them \cref{def:row_and_column_rank}, used from
\cref{thm:rank_row_column_equality} onwards but never introduced, and the whole
closing section on the transpose, whose notation $A\transp$ the transcript
already used here and in fourteen later chapters without ever defining it.

Two things go beyond the notes, and it is worth knowing which.
\Cref{ailem:column_relations_preserved} is not in Prof. Biran's text: the
transcript asserts the preservation of linear relations between columns in a
remark and then rests \cref{thm:basis_col_space} on it, so it is stated and
proved here as an AI-Lemma, on the strength of
\cref{thm:ero_preserve_solutions}. And the notes defer
$\rankrow(A) = \rankcol(A)$ to a later lecture, while this chapter already
reaches it in \cref{thm:rank_equality}; the argument is sound and is kept.

One thing is left as it stands, deliberately. The rank of a matrix is named
inside the statement of \cref{thm:rank_equality} rather than in a definition of
its own. That is unusual, but the notion stays citable, that theorem being
numbered, and nothing later appeals to a definition of matrix rank that does not
exist: the two places in ch. 17c that say \qt{by definition, $\rank(\cdot)$} mean
the rank of a linear map, which \cref{def:rank_linear_map} supplies.
\end{ainote}

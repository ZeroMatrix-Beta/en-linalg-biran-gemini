...
\end{enumerate}
The elements of $K$ are called \newterm[scalar]{scalars} and the elements of $V$ are called \newterm[vector]{vectors}.
\end{definition}
...
\begin{enumerate}[label=\textbf{(\alph*)}]
    \item The $0$ vector from \textbf{(V2)} is unique. To avoid confusion (e.g., with $0 \in K$), we will sometimes denote it by $0_V$.
    \item The element $v'$ (corresponding to $v$) from \textbf{(V3)} is unique. We will denote it by $-v$. We will also write $v - w := v + (-w)$ for all $v, w \in V$.
...
\begin{proof}
\begin{enumerate}[label=\textbf{(\alph*)}]
    \item Assume there are two such zero elements, $0_1$ and $0_2$. Then
\[
0_1 \overset{\text{\textbf{(V2)}}}{=} 0_1 + 0_2
    \overset{\text{\textbf{(V4)}}}{=} 0_2 + 0_1
    \overset{\text{\textbf{(V2)}}}{=} 0_2,
\]
the first step using that $0_2$ is a zero element and the last that $0_1$ is. Thus, $0_1 = 0_2$.

    \item Assume there are two inverses $v_1'$ and $v_2'$ for an element $v$, so that $v + v_1' = 0_V$ and $v + v_2' = 0_V$. Then we can write:
\[
v_1' \overset{\text{\textbf{(V2)}}}{=} v_1' + 0_V
     \overset{\text{\textbf{(V3)}}}{=} v_1' + (v + v_2')
     \overset{\text{\textbf{(V1)}}}{=} (v_1' + v) + v_2'
     \overset{\text{\textbf{(V4)}}}{=} 0_V + v_2'
     \overset{\text{\textbf{(V2)}}}{=} v_2',
\]
where the second step substitutes the defining property of $v_2'$ and the fourth that of $v_1'$, the latter read backwards through commutativity. Thus, the inverse is unique.

    \item Consider
\begin{equation}
\label{eq:zero_scalar_doubles}
0 \cdot v = (0 + 0) \cdot v \overset{\text{\textbf{(V8)}}}{=} 0 \cdot v + 0 \cdot v.
\end{equation}
Adding $-(0 \cdot v)$ to both sides cancels one copy:
\[
0_V \overset{\text{\textbf{(V3)}}}{=} 0 \cdot v + \bigl(-(0 \cdot v)\bigr)
    \overset{\eqref{eq:zero_scalar_doubles}}{=} \bigl(0 \cdot v + 0 \cdot v\bigr) + \bigl(-(0 \cdot v)\bigr)
    \overset{\text{\textbf{(V1)}}}{=} 0 \cdot v + \underbrace{\Bigl(0 \cdot v + \bigl(-(0 \cdot v)\bigr)\Bigr)}_{=\; 0_V}
    \overset{\text{\textbf{(V2)}}}{=} 0 \cdot v.
\]

    \item Similarly, $\alpha \cdot 0_V = \alpha \cdot (0_V + 0_V) = \alpha \cdot 0_V + \alpha \cdot 0_V$ by \textbf{(V7)}. Adding $-(\alpha \cdot 0_V)$ yields $0_V = \alpha \cdot 0_V$.
    ...
    \item By definition, $-(-v)$ is the unique element that added to $-v$ gives $0_V$. But $v$ itself satisfies $(-v) + v = v + (-v) = 0_V$ by \textbf{(V4)} and \textbf{(V3)}, so uniqueness in part \textbf{(b)} forces $-(-v) = v$.
    ...
    \[
    v \overset{\text{\textbf{(V6)}}}{=} 1 \cdot v = (a^{-1} \cdot a) \cdot v \overset{\text{\textbf{(V5)}}}{=} a^{-1} \cdot (a \cdot v) = a^{-1} \cdot 0_V \overset{\text{\textbf{(d)}}}{=} 0_V.
    \]
    So either $a = 0$ or $v = 0_V$, as claimed.

    \item This follows from \textbf{(V1)} by induction on $n$, exactly as for the associativity of addition in a field. From now on we will therefore write $v_1 + \dots + v_n$ without brackets.
\end{enumerate}
\end{proof}
...
\exsol{sol:extended_reals_not_vector_space}
\end{exercise}
...
With these operations $K^n$ becomes a vector space over $K$; the proof rests entirely on the fact that $K$ itself satisfies the field axioms. We call $K^n$ also the \newterm[coordinate space@$n$-dimensional coordinate space]{$n$-dimensional coordinate space} over $K$.
\end{example}

\begin{exercise}[The Vector Space Axioms for $K^n$]
\label{exc:kn_axioms}
Check the details: verify each of the eight axioms \textbf{(V1)}--\textbf{(V8)} for $K^n$.
...
\exsol{sol:positive_reals_vector_space_mc}
\end{exercise}
...
\begin{proof}
Conditions \textbf{(LSS2)} and \textbf{(LSS3)} say precisely that the operations $+$ and $\cdot$ of $V$ restrict to operations
\[
...
The one axiom that deserves a word is \textbf{(V2)}, because it asserts the \emph{existence} of an element of $U$ rather than an identity. Pick any element $u \in U$; this is possible because $U \ne \emptyset$ by \textbf{(LSS1)}. By \textbf{(LSS3)},
\[
0_V = 0_K \cdot u \in U,
\]
where the first equality is \cref{lem:vector_space_basics} \textbf{(c)}. Now $0_V + w = w$ holds for every $w \in U$, because it already holds for every $w \in V$ by \textbf{(V2)} for $V$. Hence, $0_V$ serves as the zero element of $U$.
\end{proof}

\begin{exercise}[The Remaining Seven Axioms for a Linear Subspace]
\label{exc:subspace_axioms}
Carry out the direct verification of the remaining seven axioms for $U$.
\exinfo{This exercise is taken from Prof.~Biran's handwritten lecture notes.}
\exsol{sol:subspace_axioms}
\end{exercise}

In practice one rarely checks \textbf{(LSS1)}--\textbf{(LSS3)} separately. The following criterion compresses them into two conditions, one of which is a single membership test.
...
\begin{proof}
\textbf{Direction 1 ($\implies$):} Suppose $U \subseteq V$ is a subspace. Then $0_V \in U$ was established in the proof of \cref{lem:subspace_is_vector_space}, which gives \textbf{(a)}. For \textbf{(b)}, given $\alpha_1, \alpha_2 \in K$ and $u_1, u_2 \in U$, condition \textbf{(LSS3)} gives $\alpha_1 u_1 \in U$ and $\alpha_2 u_2 \in U$, and then \textbf{(LSS2)} gives $\alpha_1 u_1 + \alpha_2 u_2 \in U$.

\textbf{Direction 2 ($\impliedby$):} Suppose \textbf{(a)} and \textbf{(b)} hold. Then $U \ne \emptyset$ by \textbf{(a)}, which is \textbf{(LSS1)}. Let $u_1, u_2 \in U$. By \textbf{(b)} applied with $\alpha_1 = \alpha_2 = 1$,
\[
\underbrace{1 \cdot u_1 + 1 \cdot u_2}_{= \; u_1 + u_2} \in U,
\]
which is \textbf{(LSS2)}. Finally, let $\alpha \in K$ and $u \in U$. By \textbf{(b)} applied with $\alpha_1 = \alpha$, $\alpha_2 = 0$ and $u_1 = u_2 = u$,
\[
\underbrace{\alpha \cdot u + 0 \cdot u}_{= \; \alpha \cdot u} \in U,
\]
which is \textbf{(LSS3)}. Hence, $U$ is a linear subspace.
\end{proof}
...
\exsol{sol:verifying_subspaces_r_f2}
\end{exercise}
...
\exsol{sol:subspace_identification_mc}
\end{exercise}
...
\begin{exercise}[The Converse: $U_0$ is a Linear Subspace]
\label{exc:U_b_converse}
Prove the direction $\impliedby$ of the claim on \cpageref{claim:U_b_subspace} that $U_b$ is a subspace exactly for $b = 0$: show that if $b = 0$, then $U_0$ satisfies \textbf{(LSS1)}, \textbf{(LSS2)} and \textbf{(LSS3)}, and is therefore a linear subspace of $K^3$.
\exinfo{This exercise is taken from Prof.~Biran's handwritten lecture notes.}
\exsol{sol:U_b_converse}
\end{exercise}
...
\exhint[Official Hint]{Consider different choices for the base field $K$, such as $\mathbb{F}_2$.}
\exinfo{This exercise is Problem 6 of Problem Sheet 5, Linear Algebra I, Autumn Semester 2025.}
\exsol{sol:union_three_subspaces}
\end{exercise}
...
\begin{exercise}[Proof of \cref{lem:matrix_vector_linearity} \textbf{(b)}]
\label{exc:matrix_vector_scalar}
Prove statement \textbf{(b)} of \cref{lem:matrix_vector_linearity}.
\exinfo{This exercise is taken from Prof.~Biran's handwritten lecture notes.}
\exsol{sol:matrix_vector_scalar}
\end{exercise}
...
\]
and therefore, $a \cdot x \in L$. This shows $L$ is a subspace.
...
\exsol{sol:parametric_linear_system_subspace}
\end{exercise}
...
\begin{exercise}[The Vector Space Axioms for the Sequence Space $K^\infty$]
...
\begin{exercise}[The Vector Space Axioms for the Function Space $K^S$]
...
\exsol{sol:even_odd_function_subspaces}
\end{exercise}
...
\exsol{sol:power_set_f2_vector_space}
\end{exercise}
...
\begin{exercise}[Continuous Functions Form a Linear Subspace]
...
\begin{exercise}[The Vector Space Axioms for the Polynomial Space {$K[x]$}]
...
$K[x]_d \subseteq K[x]$ is a linear subspace of $K[x]$. (Proof: on \cpageref{sol:bounded_degree_subspace}).
\end{claim*}

Later, we will see that for $d \ge 0$ the space $K[x]_d$ has dimension $d+1$. The remaining case is degenerate: $\deg(f) \le -\infty$ holds for the zero polynomial alone, so $K[x]_{-\infty} = \{0_{K[x]}\}$, of dimension $0$.
\end{example}
...
With these operations, $\M_{m \times n}(K)$ is a vector space over $K$. Its zero vector is the $m \times n$ matrix all of whose entries are $0$. (Proof: on \cpageref{sol:matrix_space}).
\end{claim*}
\end{example}

\begin{ainote}
Two notational choices here differ from Prof. Biran's and are worth registering
before they cause trouble later. Sequences are indexed from $0$ throughout this
book, so that $K^\infty = \{(a_0, a_1, \dots)\}$ and the space $\Fib$ above is
literally the one built in \cref{prop:fib_closure}, where the notes index from
$1$ and start the recursion at $k \ge 3$. And a subspace is written $U$ here
where the notes write $W$, since the later chapters need $W$ for a second space
and sums such as $U + W$ have to stay readable.

Of the eight axioms, one is easy to dismiss as bookkeeping and should not be.
\textbf{(V1)} to \textbf{(V4)} make $(V, +)$ an abelian group and say nothing
about $K$ at all, while \textbf{(V5)}, \textbf{(V7)} and \textbf{(V8)} are
compatibility conditions that even a completely trivial scalar multiplication
would satisfy: setting $\alpha \cdot v := 0_V$ for every $\alpha$ and every $v$
meets all three. \textbf{(V6)}, $1 \cdot v = v$, is the only axiom that
excludes this, and therefore the only one that forces the scalars to act on $V$
at all.

\Cref{lem:vector_space_basics} is where the axioms begin to pay, and its
statements are used silently for the rest of the book rather than cited. Part
\textbf{(g)}, that $a \cdot v = 0_V$ forces $a = 0$ or $v = 0_V$, stands behind
every argument that divides through by a coefficient, the linear independence
arguments of ch. 9 above all. Part \textbf{(h)}, generalized associativity, is
what makes the notation $\sum_{i=1}^n a_i v_i$ legitimate in the first place,
since \textbf{(V1)} on its own brackets only three summands.
\end{ainote}
...
Now look at any one of the eight axioms, say \textbf{(V1)}, $u + (v + w) = (u + v) + w$. Two elements of the space are equal exactly when they agree at every index, so the axiom holds if and only if
\[
    u(i) + \bigl(v(i) + w(i)\bigr) = \bigl(u(i) + v(i)\bigr) + w(i) \qquad \text{for every } i \in I,
\]
and that is the associativity axiom \textbf{(K1)} of the field $K$, applied once for each $i$. The same argument settles the other seven: each of \textbf{(V1)}--\textbf{(V8)} is an identity between two expressions built from $+$ and $\cdot$, both sides evaluate index by index, and the resulting identity in $K$ is one of \textbf{(K1)}--\textbf{(K10)}. Concretely, \textbf{(V1)} and \textbf{(V3)} come from \textbf{(K1)} and \textbf{(K2)}; the zero element is the function with $0$ at every index and the inverse of $u$ is $i \mapsto -u(i)$, so \textbf{(V2)} and \textbf{(V4)} come from \textbf{(K3)} and \textbf{(K4)}; \textbf{(V5)} and \textbf{(V6)} come from \textbf{(K5)} and \textbf{(K6)}; and \textbf{(V7)} and \textbf{(V8)} come from the two halves of distributivity, \textbf{(K8)}.
...
\label{sol:subspace_axioms}
Let $U \subseteq V$ be a linear subspace and let $u, v, w \in U$ and $a, b \in K$. All eight axioms are statements about elements of $U$, and every element of $U$ is an element of $V$, where the axioms already hold. For the seven axioms that are pure identities, \textbf{(V1)} and \textbf{(V3)} through \textbf{(V8)}, there is therefore nothing to check beyond one point: that both sides of the identity are elements of $U$ at all. That is exactly what \textbf{(LSS2)} and \textbf{(LSS3)} of \cref{def:linear_subspace} provide, since each side is built from $u, v, w$ by additions and scalar multiplications.

Two of them do assert existence and so deserve a word. For \textbf{(V2)}, the proof of \cref{lem:subspace_is_vector_space} above shows that the zero of $V$ lies in $U$ and serves as a zero there. For \textbf{(V4)}, given $u \in U$ the element $(-1) \cdot u$ lies in $U$ by \textbf{(LSS3)}, and it satisfies $u + (-1) \cdot u = 0_V$ by \cref{lem:vector_space_basics}, so it is an additive inverse of $u$ inside $U$.
...
\label{sol:U_b_converse}
Assume $b = 0$, so that $U_0 = \{(x_1, x_2, x_3) \in K^3 \mid x_1 + x_2 + x_3 = 0\}$. We verify the three conditions of \cref{def:linear_subspace}:
\begin{enumerate}[label=\textbf{(LSS\arabic*)}]
    \item The zero vector $(0,0,0)$ satisfies $0 + 0 + 0 = 0$, so $0 \in U_0$ and $U_0 \neq \emptyset$.

    \item Let $x, y \in U_0$, so $x_1 + x_2 + x_3 = 0$ and $y_1 + y_2 + y_3 = 0$. Then
    \[
    ...
    \item Let $x \in U_0$ and $a \in K$. Then
    \[
...
\label{sol:matrix_vector_scalar}
...
\begin{enumerate}[label=\textbf{(LSS\arabic*)}]
    \item The constant function $0$ is continuous at every point of $[0,1]$, since $|0 - 0| = 0 < \varepsilon$ for every $\varepsilon > 0$ and every choice of $\delta$. So, $0 \in C(S)$.

    \item Let $f, g \in C(S)$ and let $x_0 \in [0,1]$. Given $\varepsilon > 0$, continuity of $f$ and of $g$ at $x_0$ supplies $\delta_1, \delta_2 > 0$ with $|f(x) - f(x_0)| < \varepsilon/2$ for $|x - x_0| < \delta_1$ and $|g(x) - g(x_0)| < \varepsilon/2$ for $|x - x_0| < \delta_2$. Put $\delta := \min(\delta_1, \delta_2)$. For $|x - x_0| < \delta$ the triangle inequality gives
    \[
...
\label{sol:bounded_degree_subspace}
...
\label{sol:matrix_space}
This is the first solution above once more, with index set $I = \{1, \dots, m\} \times \{1, \dots, n\}$: a matrix is exactly a function assigning an element of $K$ to each pair $(i,j)$, and both operations are defined entry by entry. Every one of \textbf{(V1)}--\textbf{(V8)} is therefore an identity checked at each entry separately, where it becomes one of the field axioms. The zero vector is the matrix with $0$ in every entry, as claimed, and the additive inverse of $(a_{ij})$ is $(-a_{ij})$.
...
\input{content/07-vector-spaces-solutions.tex}

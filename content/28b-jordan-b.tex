...
%
% This whole file was transcribed from the scans by Opus 5 (High); there was
...
Let $W$ be a vector space over $K$ and $S \in \End(W)$. A subspace $\mathcal{U} \subseteq W$ is called \newterm{$S$-invariant} if $S(\mathcal{U}) \subseteq \mathcal{U}$, that is, if $Su \in \mathcal{U}$ for all $u \in \mathcal{U}$. If $\mathcal{U}$ is $S$-invariant, then $S$ defines an endomorphism $S|_{\mathcal{U}} \colon \mathcal{U} \to \mathcal{U}$.
\end{definition}
...
By \cref{exc:char_poly_restriction_divides}, $P_{T_\lambda}(x)$ divides $P_T(x)$. It is therefore enough to show that $P_{T_\lambda}(x)$ has no zeroes other than $\lambda$, because this will imply that $P_{T_\lambda}(x)$ divides $(\lambda - x)^{m_a(\lambda)}$, which is exactly the assertion.
...
\begin{proof}
The first statement follows from \cref{exc:nilpotent_properties}, applied to the restriction of $T - \lambda \id_V$ to $\gEig_T(\lambda)$, which is nilpotent on that subspace.
...
\]
from \cref{lem:char_poly_on_generalized_eigenspace}. Multiplying these characteristic polynomials over all eigenvalues recovers $P_T(x)$, and comparing the exponent of $(\lambda - x)$ on both sides gives $\dim \gEig_T(\lambda) = m_a(\lambda; T)$.
\end{proof}
...
\]
and we have that $N_\lambda$ sends $\gEig_T(\lambda)$ to $\gEig_T(\lambda)$, so $N_\lambda \in \End\left(\gEig_T(\lambda)\right)$. Also, $P_{N_\lambda}(x)$ splits as a product of linear factors, because $P_T(x)$ has this property. Consequently,
\[
...
Now apply $N^{k-2}$ to \eqref{eq:jordan_chain_relation}. By the same reasoning all terms vanish except the one involving $e_{k-1}$, giving
\[
...
\begin{exercise}[The Modified Family is a Basis]
...
Assume that $A$ and $A'$ are two matrices in Jordan Normal Form that both represent $T \in \End(V)$. Consequently, $P_A(x) = P_T(x) = P_{A'}(x)$.
...
\]
because the complementary part of $C$, namely the Jordan Normal Form with all eigenvalues different from $0$, is invertible and so contributes nothing to the kernel. Similarly, for every $k$,
\[
    \dim \ker S^k = \dim \ker (C')^k = \dim \ker (D')^k.
\]
Consequently,

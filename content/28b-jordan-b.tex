...
\[
    P_{T_\lambda}(x) = (\lambda - x)^m \qquad \text{and} \qquad P_{T_\mu}(x) = (\mu - x)^{m'},
\]
again by \cref{exc:char_poly_restriction_divides} (for some $m, m' \ge 1$). This is impossible, because $\lambda \neq \mu$ and a non-constant polynomial cannot divide two coprime polynomials. Consequently, $\dim \mathcal{U} = 0$, that is, $\mathcal{U} = \{0\}$.
\end{proof}
...
\begin{example}[Nilpotency of Standard Jordan Blocks]
...
\[
    \mathcal{B}_3 = (e_1, \dots, e_k, e_{k+1}, \dots, e_{k+m}, f_{m+1}, \dots, f_\ell)
\]
are linearly independent, and hence form a basis for $V$. (Prof. Biran's hint: $e_{k+j} = f_j + (\text{something from } W)$).
\exinfo{This exercise is taken from Prof.~Biran's handwritten lecture notes.}

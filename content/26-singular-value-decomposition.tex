...
Indeed, $B$ is self adjoint ($B^* = (A^* A)^* = A^* (A^*)^* = A^* A = B$), hence $\eta \in \mathbb{R}$. To see that $\eta > 0$, let $v$ be an eigenvector for $\eta$, so that $v \neq 0$ and $Bv = \eta v$. Substituting this into \eqref{eq:svd_pos_def} gives
\[
...
By a previous theorem, $B$ is orthogonally diagonalizable, hence there exists $Q \in \Orth(n)$ (or $\Unit(n)$) such that
\[

...
\defterm{singular value decomposition}
...
% Creator: Opus 5
\begin{ainote}
This section is two pages and a single theorem, and the whole of it turns on one
step that is easy to read past. The proof calls the diagonal entries $\eta_j$ of
$Q^{-1} B Q$ positive reals, on the strength of a paragraph above it that speaks
about the \emph{eigenvalues} of $B$. Those two lists agree because $Q^{-1} B Q$
and $B$ are similar and therefore share a characteristic polynomial
(\cref{lem:properties_char_poly} \textbf{(e)}), whose roots are exactly the
eigenvalues (\cref{aiprop:eigenvalue_iff_root}). Without that identification
$\lambda_j := +\sqrt{\eta_j}$ is not yet known to be defined at all, and with it
the singular values come for free.

Prof. Biran numbers nothing in this section. The single theorem is printed as
26.a.1 regardless, which shifts nothing elsewhere and keeps the result citable;
the header of the file records that decision.
\end{ainote}

...
\end{enumerate}
\exhint{For part (b), if $\dim W = 3$, find a non-zero normal vector $n = (n_1, n_2, n_3, n_4)\transp \in \mathbb{R}^4$ orthogonal to all $w \in W$ (i.e., $\sum_{i=1}^4 n_i w_i = 0$ for all $w \in W$). Then $W$ is described by the single equation $n_1 x_1 + n_2 x_2 + n_3 x_3 + n_4 x_4 = 0$.}
...
\]
\exhint{Recall from analysis that the only continuous solution to the initial value problem $y'' + y = 0$ with $y(0) = 0$ and $y'(0) = 0$ is the identically zero function $y \equiv 0$. Consider evaluating functions and their first derivatives at $0$.}

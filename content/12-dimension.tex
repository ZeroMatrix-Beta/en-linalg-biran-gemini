...
\begin{ainote}
\Cref{thm:wrong_length_lists} is stated and proved in the notes and was missing
from this transcript; it is restored above. The notes justify the two
independence steps in the proof of
\cref{prop:subspace_dimension_constraints} by citing Lemma E of the previous
chapter (\cref{lem:no_predecessor_span}), which the transcript appealed to
without naming; the citations are put back, leaving the surrounding wording as
it was. The trivial direction of the equality claim in
\cref{prop:subspace_dimension_constraints}, that $U = V$ gives
$\dim U = \dim V$, was also missing and is supplied. The notes restate
\cref{lem:no_predecessor_span} in full at this point as a recollection from the
earlier lecture; it is only cross-referenced here.

The rigour pass supplied the proof of \cref{sum:finite_dim_properties}. The
notes give the two principles as a recollection of the previous lecture and
prove neither, and nothing else in the book proves them either, although item
\textbf{(2)}, the basis extension property, is used some ten times from ch. 14
onwards, occasionally credited in passing to \qt{the Steinitz Exchange Lemma}.
Both follow from \cref{lem:spanning_list_contains_basis} and
\cref{lem:strong_steinitz} in a few lines. The derivation avoids
\cref{thm:basis_equivalent_characterizations}, which stands below it and whose
own proof appeals to both items; going through it would be circular.

Also: the converse half of the equality claim in
\cref{prop:subspace_dimension_constraints} opened with \qt{Furthermore}, which
announces a further fact rather than the converse that the sentence above it had
just set up, and the independent list it produces is a basis of $U$, which is
what makes it independent in $V$ as well. Four silent steps now name their
reason, among them \cref{thm:wrong_length_lists} \textbf{(2)}, which the proof
of \cref{prop:subspace_dimension_constraints} restates in words although it
stands two results above.
\end{ainote}

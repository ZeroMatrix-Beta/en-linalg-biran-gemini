...
% Creator: Opus 5
% Neither item is proved anywhere in the book, and item (b), the basis
...
\textbf{(a)} This is \cref{lem:spanning_list_contains_basis}, which extracts a basis from within the list $(v_1, \dots, v_n)$ itself. Reading its elements in increasing order of index gives the sublist $(v_{i_1}, \dots, v_{i_d})$, and $d = \dim V$ by \cref{def:dimension}, since every basis of $V$ has that many elements.

\textbf{(b)} Let $(u_1, \dots, u_m)$ be linearly independent, and let $\mathcal{B} = (b_1, \dots, b_d)$ be a basis of $V$, which exists and has $d = \dim V$ elements by \cref{thm:basis_theorem}. In particular, $\Sp(b_1, \dots, b_d) = V$, so \cref{lem:strong_steinitz} applies to the independent list $(u_1, \dots, u_m)$ and the spanning list $\mathcal{B}$. It yields $m \le d$ together with $d - m$ survivors $w_1, \dots, w_{d-m}$ of $\mathcal{B}$ such that
\[
...
\begin{proof}
\textbf{(1)} Assume that $k < n$, and suppose by contradiction that $\Sp(v_1, \dots, v_k) = V$. By item \textbf{(a)} of the summary on \cpageref{sum:finite_dim_properties} we can delete some of the vectors from the list $v_1, \dots, v_k$ and obtain a basis $v_{i_1}, \dots, v_{i_{k'}}$ of $V$. Then
\[
...
\textbf{(2)} Assume that $k > n$, and suppose by contradiction that $v_1, \dots, v_k$ are linearly independent. By item \textbf{(b)} of the summary on \cpageref{sum:finite_dim_properties} we can extend this list to a basis of $V$. Then
\[
\dim V \ge k > n = \dim V,
\]
...
\end{enumerate}
\exhint[Official Hint]{For part \textbf{(b)}, if $\dim W = 3$, find a non-zero normal vector $n = (n_1, n_2, n_3, n_4)\transp \in \mathbb{R}^4$ orthogonal to all $w \in W$ (i.e., $\sum_{i=1}^4 n_i w_i = 0$ for all $w \in W$). Then $W$ is described by the single equation $n_1 x_1 + n_2 x_2 + n_3 x_3 + n_4 x_4 = 0$.}
...
\begin{ainote}
The two principles collected in \cref{sum:finite_dim_properties} are recalled in
the notes rather than proved, and nothing else in the book proves them, although
the second of them, the basis extension property, is used some ten times from
ch. 14 onwards and is occasionally credited in passing to \qt{the Steinitz
Exchange Lemma}. Both are proved above, in a few lines each, from
\cref{lem:spanning_list_contains_basis} and \cref{lem:strong_steinitz}.

The route those two proofs take is longer than it needs to look, and
deliberately so. The natural argument would run through
\cref{thm:basis_equivalent_characterizations}, which stands just below and
delivers both principles at once; but that theorem's own proof appeals to both
of them in turn, so the short route is circular. Anyone tempted to compress the
argument should check that dependency first.

\Cref{thm:wrong_length_lists} is the statement to read alongside them: a list
shorter than $\dim V$ cannot span, and a list longer than $\dim V$ cannot be
independent. That is what turns the dimension from a number attached to a space
into a constraint on every list inside it, and most of the counting arguments of
the later chapters are it in disguise.
\end{ainote}

\input{content/12-dimension-solutions.tex}

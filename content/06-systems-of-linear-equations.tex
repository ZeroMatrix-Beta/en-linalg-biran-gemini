...
\begin{cases}
E_1: \quad x + 3y + 4z = -1 \\
E_2: \quad 2x - \phantom{3}y + \phantom{4}z = \phantom{-}5
\end{cases}
\end{equation*}

We operate on the equations so as to eliminate variables one at a time. First we remove $x$ from the second equation by $-2 \cdot E_1 + E_2 \to E_2$:
\begin{equation*}
\begin{cases}
x + 3y + 4z = -1 \\
...
\end{cases}
\end{equation*}
Next we normalise the second equation by $-\frac{1}{7} \cdot E_2 \to E_2$:
\begin{equation*}
\begin{cases}
x + 3y + 4z = -1 \\
...
\end{cases}
\end{equation*}
Finally, instead of substituting backwards, we eliminate $y$ \emph{upwards} as well, using $-3 \cdot E_2 + E_1 \to E_1$:
\begin{equation*}
\begin{cases}
...
\begin{figure}[ht!]
  ...
  \begin{tikzpicture}[scale=0.92, font=\small]
    ...
    \node[ThemeSecondary!75!black, anchor=south east] at (-4.6,1.5) {$E_1$};
    \node[ThemeGreen!62!black, anchor=south west] at (4.9,1.1) {$E_2$};
  \end{tikzpicture}
...
Suppose we append a third equation $E_3: 3x + 2y + z = 0$ to the system above. Substituting the parametrised solution \eqref{eq:motivating_solution_set} into it gives
\[
...
Let $E_i$ denote the $i$-th equation of a system, equivalently the $i$-th row of its augmented matrix. The three \newterm{elementary row operations} (EROs) are:
\begin{enumerate}[label=\textbf{(\alph*)}]
...
\textbf{(b)} Operation $\lambda \cdot E_i + E_j \to E_j$. Again all equations of $(S)$ and $(S')$ coincide except for equation $j$:
\begin{align*}
...
\textbf{(c)} Operation $E_i \leftrightarrow E_j$. Here we clearly have
\[
...
\begin{enumerate}[label=\textbf{(\alph*)}]
    \item If $(S) \xrightarrow{\;\lambda \cdot E_i \to E_i\;} (S')$ with $\lambda \ne 0$, then $(S') \xrightarrow{\;\frac{1}{\lambda} \cdot E_i \to E_i\;} (S)$.
    \item If $(S) \xrightarrow{\;\lambda \cdot E_i + E_j \to E_j\;} (S')$ with $i \ne j$, then $(S') \xrightarrow{\;-\lambda \cdot E_i + E_j \to E_j\;} (S)$.
    \item If $(S) \xrightarrow{\;E_i \leftrightarrow E_j\;} (S')$, then $(S') \xrightarrow{\;E_j \leftrightarrow E_i\;} (S)$.
\end{enumerate}
...
& \vdots & & \vdots &
\end{pmatrix}
\xrightarrow{\; -a_{ik} \cdot E_1 + E_i \to E_i \;}
\begin{pmatrix}
0 & \dots & 0 & 1 & \dots \\
& \vdots & & \vdots & \\
...
\[
\xrightarrow{-\frac{1}{9} \cdot E_1 \to E_1}
\begin{pmatrix}
0 & 1 & -\frac{1}{3} & -\frac{4}{9} & \big| & -1 \\
1 & 4 & 0 & -1 & \big| & 5 \\
2 & 6 & -1 & 5 & \big| & -5
\end{pmatrix}
\xrightarrow{\begin{subarray}{l} -4 \cdot E_1 + E_2 \to E_2 \\ -6 \cdot E_1 + E_3 \to E_3 \end{subarray}}
\begin{pmatrix}
0 & 1 & -\frac{1}{3} & -\frac{4}{9} & \big| & -1 \\
...
\[
\xrightarrow{-2 \cdot E_2 + E_3 \to E_3}
\begin{pmatrix}
0 & 1 & -\frac{1}{3} & -\frac{4}{9} & \big| & -1 \\
1 & 0 & \frac{4}{3} & \frac{7}{9} & \big| & 9 \\
0 & 0 & -\frac{5}{3} & \frac{55}{9} & \big| & -17
\end{pmatrix}
\xrightarrow{-\frac{3}{5} \cdot E_3 \to E_3}
\begin{pmatrix}
0 & 1 & -\frac{1}{3} & -\frac{4}{9} & \big| & -1 \\
1 & 0 & \frac{4}{3} & \frac{7}{9} & \big| & 9 \\
...
\[
\xrightarrow{\begin{subarray}{l} -\frac{4}{3} \cdot E_3 + E_2 \to E_2 \\ \frac{1}{3} \cdot E_3 + E_1 \to E_1 \end{subarray}}
\begin{pmatrix}
...
\end{pmatrix}
\xrightarrow{\frac{1}{5} \cdot E_2 \to E_2}
\begin{pmatrix}
1 & -2 & 1 & \big| & b_1 \\
...
\begin{ainote}
The lecture notes for this chapter use $F$ for the base field and $R_i$ for the
rows; this transcript writes $K$ and $E_i$ throughout, in line with the
conventions used in the rest of the document. The two matrices in the schematic
step of the proof of \cref{thm:row_reduction_exists}, and the final example above,
are expanded slightly beyond what the notes show; the geometric picture in
\cref{fig:two_planes} and the staircase in \cref{fig:echelon_staircase} were added
here. Everything else follows the notes directly.
\end{ainote}

\begin{ainote}
This chapter is the one the rest of the book keeps coming back to, so it is worth
saying which of its questions stay open at the end of it.

The three worked examples exhibit three different answers, and none of the three
is explained here. \Cref{ex:full_reduction} has a one-parameter family of
solutions, \cref{ex:consistency_conditions} has either none or a one-parameter
family depending on the constants, and \cref{ex:infinitely_many_solutions} ends
with the solution set written as one particular solution plus a multiple of a
fixed vector. Why those are the only shapes available, why the number of free
parameters is $n$ minus the number of pivots, and why the number of pivots does
not depend on which sequence of row operations was used, are all questions this
chapter raises and cannot answer. They are settled in ch. 13 and ch. 17d, once
the rank of a matrix exists.

What the chapter does establish is the one thing the method needs:
\cref{thm:ero_preserve_solutions}, that row operations change nothing about the
solution set. Everything else here is bookkeeping in the service of that theorem,
and its proof, through the three `claim*` blocks, is the model for the
reversibility arguments used again in ch. 17e.

The rigour pass found nothing false. All three worked examples were recomputed
entry by entry and the final answers substituted back into the original systems;
they are correct, including the fractions in \cref{ex:full_reduction}. The
examples and the augmented-matrix Notation were given labels so that the later
chapters can point at them, and the reference to \qt{our \textbf{(c)} question}
now names the list of goals it means and the corollary in ch. 17d that keeps the
promise.
\end{ainote}

...
\begin{ainote}
The point of this chapter is easy to lose behind the ten axioms, so it is worth
stating: from here on the course never again asks what kind of number a scalar
is. Every result in the book is proved over an unspecified field $K$, and the
two lemmata just proved are the model for that. They look obvious over
$\mathbb{R}$, and neither proof uses anything about $\mathbb{R}$; they are
therefore true in $\mathbb{F}_2$ as well, where \qt{obvious} would be a poor
guide.

That generality is not free, and two places in the book pay for it. The
Polarization Identity of ch. 27 needs $2 \neq 0$ in $K$, which fails in
$\mathbb{F}_2$, and so does everything downstream of it about symmetric bilinear
forms. And the whole of chs. 22 to 26 works over $\mathbb{R}$ or $\mathbb{C}$
only, because an inner product needs an order on the scalars in which squares are
non-negative, which no finite field has. Where a chapter names its field, it is
usually for one of these two reasons.

The rigour pass proved \cref{thm:fp_is_field}, which the chapter stated without
one, and gave it a reading under which it has something to decide: as written it
speaks about $\mathbb{F}_p$, which \cref{def:field_fp} has already declared to
have $p$ prime. The informal discussion after it argues one direction, and only
for the single modulus $4$; the other direction, that a prime modulus really does
supply inverses, needs Bézout and is the half that makes finite fields exist at
all.
\end{ainote}

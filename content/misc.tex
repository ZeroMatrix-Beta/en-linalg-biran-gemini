...
\begin{definition}[Affine Space]
  A subset $X \subseteq V$ is called an \textbf{affine space} if there exists a linear subspace $U \subseteq V$ and a translation vector $x_0 \in V$ such that $X = x_0 + U$.

  We say that $X$ is modeled on the subspace $U$, and we write the direction space as $\vec{X} := U$. The elements of $X$ are called \textit{points}, while the elements of $\vec{X}$ are called \textit{vectors}.
\end{definition}
...
\begin{definition}[Eigenvalues and Eigenvectors]
  Let $V$ be a vector space and $T \in \End(V)$. A scalar $\lambda \in K$ is called an \textbf{eigenvalue} of $T$ if there exists a non-zero vector $v \in V$ such that:
  \[ T(v) = \lambda v. \]
  The vector $v$ is called an \textbf{eigenvector} associated with the eigenvalue $\lambda$.
\end{definition}

...
\begin{enumerate}[label=\textbf{(\alph*)}]
    \item \textbf{Failure of Associativity of Addition \textbf{(V1)}:}
    ...
    \item \textbf{Failure of Scalar Distributivity \textbf{(V7)}:}
    Axiom \textbf{(V7)} requires $(a + b) \cdot v = a \cdot v + b \cdot v$ for all $a, b \in \mathbb{R}$ and $v \in V$. Choose $a = 2$, $b = -1$, and $v = \infty$. Then:
    \[
    ...
    \]
    Since $\infty \ne 0$, axiom \textbf{(V7)} fails.
\end{enumerate}
Therefore, $V$ fails to be a vector space over $\mathbb{R}$.
\exinfo{Official Solution (Problem Sheet 5, Exercise 4). Checks failure of vector space axioms for extended real numbers.}
\end{exercisesolution}

% Official Solution (Problem Sheet 5, Multiple Choice Question 2)
...
\begin{enumerate}[label=\textbf{(\arabic*)}]
    ...
    This fails axiom \textbf{(V6)}, which requires $1 \odot v = v$. Indeed,
    \[
    1 \odot \begin{pmatrix} x_1 \\ x_2 \end{pmatrix} = \begin{pmatrix} e x_1 \\ e x_2 \end{pmatrix} \ne \begin{pmatrix} x_1 \\ x_2 \end{pmatrix}.
    \]
    It also fails scalar distributivity \textbf{(V7)} because $(\lambda + \mu) \odot v$ would have components $e^{\lambda+\mu} x_i$, whereas $(\lambda \odot v) \oplus (\mu \odot v)$ has components $(e^\lambda x_i)(e^\mu x_i) = e^{\lambda+\mu} x_i^2$.
    ...
    \begin{itemize}
        ...
        \item \textbf{Axiom \textbf{(V5)}:} $\lambda \odot (\mu \odot x) = \lambda \odot (x_1^\mu, x_2^\mu) = ((x_1^\mu)^\lambda, (x_2^\mu)^\lambda) = (x_1^{\lambda\mu}, x_2^{\lambda\mu}) = (\lambda\mu) \odot x$.
        \item \textbf{Axiom \textbf{(V6)}:} $1 \odot x = (x_1^1, x_2^1) = (x_1, x_2) = x$.
        \item \textbf{Axiom \textbf{(V7)}:} $(\lambda + \mu) \odot x = (x_1^{\lambda+\mu}, x_2^{\lambda+\mu}) = (x_1^\lambda x_1^\mu, x_2^\lambda x_2^\mu) = (x_1^\lambda, x_2^\lambda) \oplus (x_1^\mu, x_2^\mu) = (\lambda \odot x) \oplus (\mu \odot x)$.
        \item \textbf{Axiom \textbf{(V8)}:} $\lambda \odot (x \oplus y) = \lambda \odot (x_1 y_1, x_2 y_2) = ((x_1 y_1)^\lambda, (x_2 y_2)^\lambda) = (x_1^\lambda y_1^\lambda, x_2^\lambda y_2^\lambda) = (x_1^\lambda, x_2^\lambda) \oplus (y_1^\lambda, y_2^\lambda) = (\lambda \odot x) \oplus (\lambda \odot y)$.
    \end{itemize}
\end{enumerate}
...
\exinfo{Official Solution (Problem Sheet 5, Multiple Choice Question 2). Identifies the power-operation scalar multiplication on $\mathbb{R}_{>0}^2$.}
\end{exercisesolution}

% Official Solution (Problem Sheet 5, Exercise 2)
...
\begin{enumerate}[label=\textbf{(\alph*)}]
    ...
    \end{itemize}
\end{enumerate}
\exinfo{Official Solution (Problem Sheet 5, Exercise 2). Verifies subspaces over $\mathbb{R}$ versus $\mathbb{F}_2$.}
\end{exercisesolution}

% Official Solution (Problem Sheet 5, Multiple Choice Question 1)
...
\end{enumerate}
\exinfo{Official Solution (Problem Sheet 5, Multiple Choice Question 1). Tests closure axioms for candidate subsets in $\mathbb{R}^n$.}
\end{exercisesolution}

% Solution: Gemini / Official Hybrid (Problem Sheet 5, Exercise 6)
...
\end{itemize}
\exinfo{Hybrid Solution (Gemini 3.7 Flash / Official Problem Sheet 5, Exercise 6). Compares union of three subspaces over $\mathbb{F}_2$ with $\mathbb{R}$.}
\end{exercisesolution}

% Solution: Gemini / Official Hybrid (Problem Sheet 5, Exercise 1)
...
\begin{enumerate}[label=\textbf{(\alph*)}]
    ...
    A linear subspace must contain the zero vector $(0, 0)$ by \textbf{(LSS1)}.
    \begin{itemize}
    ...
    \end{itemize}
\end{enumerate}
\exinfo{Hybrid Solution (Gemini 3.7 Flash / Official Problem Sheet 5, Exercise 1). Analyzes the affine solution geometry under parameter variations.}
\end{exercisesolution}
...
\end{enumerate}
\exinfo{Official Solution (Problem Sheet 5, Exercise 3). Proves direct sum decomposition into even and odd function subspaces for $\operatorname{char}(K) \ne 2$.}
\end{exercisesolution}

% Official Solution (Problem Sheet 5, Exercise 5)
\begin{exercisesolution}[Solution to \cref{exc:power_set_f2_vector_space}]
\label{sol:power_set_f2_vector_space}
We verify the eight vector space axioms \textbf{(V1)}--\textbf{(V8)} for $(\mathcal{P}(X), \bigtriangleup, \cdot, \emptyset)$ over $\mathbb{F}_2 = \{0, 1\}$. It is helpful to use the characteristic function $\chi_A \colon X \to \mathbb{F}_2$ defined by $\chi_A(x) = 1$ if $x \in A$ and $\chi_A(x) = 0$ if $x \notin A$. For symmetric difference, $\chi_{A \mathbin{\triangle} B}(x) = \chi_A(x) + \chi_B(x) \pmod 2$.
...
\begin{enumerate}[label=(\textbf{V}\arabic*), leftmargin=3.5em]
    ...
    \item \textbf{Distributivity in Scalars:}
    ...
    \item \textbf{Distributivity in Vectors:}
...
\exinfo{Official Solution (Problem Sheet 5, Exercise 5). Verifies the 8 vector space axioms for the power set $\mathcal{P}(X)$ over $\mathbb{F}_2$.}
\end{exercisesolution}

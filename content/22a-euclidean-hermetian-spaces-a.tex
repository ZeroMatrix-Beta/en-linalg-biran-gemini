...
\begin{exercisesolution}[Proof of Polarization Identity (on \cpageref{exc:reconstruct_scalar_prod} \textbf{(b)})]
    ...
    \]
    By expanding each norm term using $\|x+y\|^2 = \|x\|^2 + \|y\|^2 + 2\Rea\langle x, y \rangle$ and maintaining careful track of the imaginary components through sesquilinearity, the cross terms isolate $\langle u, v \rangle$.
\end{exercisesolution}

\begin{exercisesolution}[Positive Definite Matrices (on \cpageref{exc:pos_def_2x2_complex})]
    Let $A = \left(\begin{smallmatrix} a & b \\ \overline{b} & d \end{smallmatrix}\right) \in \M_{2 \times 2}(\mathbb{C})$ be a Hermitian matrix (which implies $a, d \in \mathbb{R}$). 
    For $v = \binom{x}{y} \in \mathbb{C}^2$, the condition $v\transp A \overline{v} > 0$ yields
    \[
        a|x|^2 + b x \overline{y} + \overline{b} \overline{x} y + d|y|^2 > 0.
    \]
    Choosing $v = \binom{1}{0}$ gives $a > 0$. Through completing the square, the full expression is positive for all non-zero $v$ if and only if $a > 0$ and $ad - |b|^2 > 0$. This confirms the condition $a > 0$ and $\det A > 0$.
\end{exercisesolution}